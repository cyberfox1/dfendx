...
uses SysUtils, CommonHelpers, CommonTools, PrgSetupUnit, PrgConsts, ImageStretch;
...
begin
  ...
  { Multi-size ICO: pick matching frame for the image list — do not ScaleImage. }
  If Ext='.ICO' then begin
    Ico:=TIcon.Create;
    try
      try
        If (ImageList.Width>0) and (ImageList.Height>0) then
          Ico.SetSize(ImageList.Width,ImageList.Height);
        Ico.LoadFromFile(FileName);
        ImageList.ReplaceIcon(Nr,Ico);
        result:=True;
      except
        result:=False;
      end;

...
Type TConfOptH=class(TBasePrgSetup)
  public
    ...
    property GlideEmulationLFBX : String index 67 read GetString write SetString;
    property GlideVoodooMem : String index 68 read GetString write SetString;
    property GlideVoodooMemPure : String index 69 read GetString write SetString;
    property GlideVoodooCardX : String index 70 read GetString write SetString;
    property GlideVoodooPerfPure : String index 71 read GetString write SetString;
    property GlideVoodooThreads : String index 72 read GetString write SetString;
    property GlideVoodooBilinear : String index 73 read GetString write SetString;
    property GlideVoodooScale : String index 74 read GetString write SetString;
    property GlideVoodooGamma : String index 75 read GetString write SetString;
    ...
    { Per-DOSBox-kind option lists (same ConfOpt multi-list pattern as Render). }
    property RenderStaging : String index 53 read GetString write SetString;
    property RenderX : String index 54 read GetString write SetString;
...
const NR_Name=1;
      ...
      NR_GlidePort=261;
      NR_GlideLFB=262;
      NR_GlideVoodooMem=294;
      NR_GlideVoodooCard=295;
      NR_GlideVoodooThreads=296;
      NR_GlideBilinear=297;
      NR_GlideVoodooScale=298;
      NR_GlideVoodooGamma=299;
      NR_GlidePurePerf=300;
      NR_PixelShader=263;
      NR_ShaderPreset=264;
      NR_VSync=265;
...
Type TGameDBH=class;
  ...
  public
    ...
    Procedure CreateConfFile; virtual;
    ...
    property GlideEnabled : Boolean index NR_GlideEnabled read GetBoolean write SetBoolean;
    ...
    property GlideVoodooMem : String index NR_GlideVoodooMem read GetString write SetString;
    property GlideVoodooCard : String index NR_GlideVoodooCard read GetString write SetString;
    property GlideVoodooThreads : String index NR_GlideVoodooThreads read GetString write SetString;
    property GlideBilinear : String index NR_GlideBilinear read GetString write SetString;
    property GlideVoodooScale : String index NR_GlideVoodooScale read GetString write SetString;
    property GlideVoodooGamma : String index NR_GlideVoodooGamma read GetString write SetString;
    property GlidePurePerf : String index NR_GlidePurePerf read GetString write SetString;
    ...
    { Staging inactive-window mode: Ord(TScreenInactiveMode) 0=do nothing, 1=mute, 2=pause. }
    property OnScreenInactive : Integer index NR_OnScreenInactive read GetInteger write SetInteger;
  ...
  protected
    ...
    function GetGame(I: Integer): TGameH;
  ...
  public
    ...
    Function Add(const AGame : TGameH) : Integer; overload;
    Function IndexOf(const AGame : TGameH) : Integer; overload;
...
function ResolveDOSBoxInstallIndex(const InstallNameOrDefault: String): Integer;
{ empty or "default" → 0; matches DOSBoxSettings[i].Name (i≥1) → i; else -1. }
function ResolveDOSBoxDir(const CustomDOSBoxDir: String): String;
{ default / named install → that install's dir (MakeAbsPath); bare path → MakeAbsPath of value.
  Returns '' only if a resolved install has no path configured. Dir need not exist on disk. }
function DefaultMapperFileForProfile(const CustomDOSBoxDir: String): String;
{ Mapper path from the profile's DOSBox install; bare path → install 0. }
function MapperBrowseInitialDir(const CustomDOSBoxDir, CurrentMapperField: String): String;
{ Folder for the mapper file dialog: current field if set, else folder of DefaultMapperFileForProfile. }
function ResolveDOSBoxKindIconPath(Kind: TDOSBoxKind): String;
{ Absolute path to kind ICO (program dir then PrgDataDir); '' if missing. }
function ResolveDOSBoxKindPreviewPath(Kind: TDOSBoxKind): String;
{ Absolute path to kind PNG (program dir then PrgDataDir); '' if missing. }
...
Constructor TGameH.Create(const ASetupFile : String);
begin
  inherited Create(ASetupFile);
  FGameDB:=nil;
  InitData;
  LoadCache;
  // trigger computation for default value via the SetString hook
  SetString(NR_CustomDOSBoxDir, GetString(NR_CustomDOSBoxDir));
end;

Constructor TGameH.CreateDelayed(const ASetupFile : String; const ATimeStampCheck : Boolean);
begin
  If ATimeStampCheck then inherited Create(ASetupFile) else inherited CreateNoTimeStampCheck(ASetupFile);
  FGameDB:=nil;
end;

constructor TGameH.Create(const ABasePrgSetup: TBasePrgSetup);
begin
...
destructor TGameH.Destroy;
begin
  {not needed: StoreAllValues;}
  inherited Destroy;
end;

procedure TGameH.InitData;
Var S : String;
    I : Integer;
begin
  ...
  AddBooleanRec(NR_GlideEnabled,'glide','active',false);
  AddStringRec(NR_GlideEmulation,'glide','emulation','false');
  AddStringRec(NR_GlidePort,'glide','port','600');
  AddStringRec(NR_GlideLFB,'glide','LFB','full');
  AddStringRec(NR_GlideVoodooMem,'voodoo','memsize','12');
  AddStringRec(NR_GlideVoodooCard,'voodoo','card','auto');
  AddStringRec(NR_GlideVoodooThreads,'voodoo','threads','auto');
  AddStringRec(NR_GlideBilinear,'voodoo','bilinear','true');
  AddStringRec(NR_GlideVoodooScale,'voodoo','scale','1');
  AddStringRec(NR_GlideVoodooGamma,'voodoo','gamma','-2');
  AddStringRec(NR_GlidePurePerf,'voodoo','pure_perf','auto');
...
Function TGameH.GetExtraPrgFile(I : Integer) : String;
begin
  result:='';
...
Function TGameH.GetExtraPrgFileParameter(I : Integer) : String;
begin
  result:='';
  If (I<0) or (I>49) then exit;
...
function TGameH.GetLicense: String;
Var I : Integer;
    St : TStringList;
begin
  result:='';
...
Procedure TGameH.SetExtraPrgFile(I : Integer; S : String);
begin
  If (I<0) or (I>49) then exit;
  SetString(NR_ExtraPrgFile+I,S);
end;

Procedure TGameH.SetExtraPrgFileParameter(I : Integer; S : String);
begin
  If (I<0) or (I>49) then exit;
  SetString(NR_ExtraPrgFileParameter+I,S);
end;

function TGameH.GetMount(I: Integer): String;
begin
...
procedure TGameH.SetMount(I: Integer; const Value: String);
begin
  Case I of
    0 : Mount0:=Value;
    1 : Mount1:=Value;
...
Function TGameH.GetWWWNameString(I : Integer) : String;
Var S : String;
begin
...
Function TGameH.GetPlainWWWNameString(I : Integer) : String;
begin
  If (I<1) or (I>9) then begin result:=''; exit; end;
  result:=GetString(NR_WWW1Name+(I-1)*2);
end;

Procedure TGameH.SetWWWNameString(I : Integer; const Value : String);
begin
  If (I<1) or (I>9) then exit;
  SetString(NR_WWW1Name+(I-1)*2,Value);
end;

Function TGameH.GetWWWString(I : Integer) : String;
begin
  If (I<1) or (I>9) then begin result:=''; exit; end;
  result:=GetString(NR_WWW1+(I-1)*2);
end;

Procedure TGameH.SetWWWString(I : Integer; const Value : String);
begin
  If (I<1) or (I>9) then exit;
  SetString(NR_WWW1+(I-1)*2,Value);
end;

procedure TGameH.LoadCache;
begin
  CacheName:=Name;
...
Procedure TGameH.CreateConfFile;
Var St : TStringList;
begin
  St:=TStringList.Create;
  try
    St.Add('# Configuration file for "'+CacheName+'"');
    St.Add('# Created by DFendX');
...
begin
  if Index = NR_CustomDOSBoxDir then begin
    ...
    if not SameText(Trim(Old), Trim(Value)) then begin
      inherited SetString(NR_PixelShader, '');
      inherited SetString(NR_ShaderPreset, '');
    end;
  end else
    inherited SetString(Index, Value);
end;
...
procedure TGameDBH.Clear;
Var I : Integer;
begin
  For I:=0 to FGameList.Count-1 do TGameH(FGameList[I]).Free;
  FGameList.Clear;
end;

Function TGameDBH.GetProfilesListFromDrive : TStringList;
Var Rec : TSearchRec;
    I : Integer;
begin
  ...
  try
    while I=0 do begin
      If (Rec.Attr and faDirectory)=0 then result.AddObject(Rec.Name,TObject(DateTimeToFileDate(Rec.TimeStamp)));
      I:=FindNext(Rec);
    end;
  finally
...
procedure TGameDBH.DeleteOldFiles;
Var Rec : TSearchRec;
    I : Integer;
begin
...
function TGameDBH.MakePROFFileName(const AName, ADir: String; const CheckExistingFiles : Boolean): String;
...
Function TGameDBH.ProfFileName(const AName : String; const CheckExistingFiles : Boolean) : String;
Var S : String;
begin
...
Function TGameDBH.Add(const AGame : TGameH; const AName : String) : Integer;
begin
  AGame.Name:=AName;
...
Function TGameDBH.Add(const AGame : TGameH) : Integer;
begin
  result:=Add(AGame, AGame.Name);
end;

function TGameDBH.IndexOf(const AGame: TGameH): Integer;
Var I : Integer;
begin
  result:=-1;
  For I:=0 to FGameList.Count-1 do If TGameH(FGameList[I])=AGame then begin result:=I; exit; end;
end;

function TGameDBH.IndexOf(const AGame: String): Integer;
Var I : Integer;
begin
  result:=-1;
  For I:=0 to FGameList.Count-1 do If TGameH(FGameList[I]).Name=AGame then begin result:=I; exit; end;
end;

function TGameDBH.GetCount: Integer;
begin
  result:=FGameList.Count;
end;

function TGameDBH.GetGame(I: Integer): TGameH;
begin
...
function TGameDBH.GetGenreList(WithDefaultProfile : Boolean; const HideWindowsProfiles : Boolean): TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).CacheGenre);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
function TGameDBH.GetDeveloperList(WithDefaultProfile : Boolean; const HideWindowsProfiles : Boolean): TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).CacheDeveloper);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
function TGameDBH.GetPublisherList(WithDefaultProfile : Boolean; const HideWindowsProfiles : Boolean): TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).CachePublisher);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
function TGameDBH.GetYearList(WithDefaultProfile : Boolean; const HideWindowsProfiles : Boolean): TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).CacheYear);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
function TGameDBH.GetLanguageList(WithDefaultProfile : Boolean; const HideWindowsProfiles : Boolean): TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).CacheLanguage);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
Function TGameDBH.GetLicenseList(WithDefaultProfile : Boolean =True; const HideWindowsProfiles : Boolean = False) : TStringList;
Var I : Integer;
    St : TStringList;
begin
  St:=TStringList.Create;
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      St.Add(TGameH(FGameList[I]).License);
    end;
    result:=GetList(St);
  finally
    St.Free;
...
Function TGameDBH.GetWWWList(WithDefaultProfile : Boolean =True; const HideWindowsProfiles : Boolean = False) : TStringList;
Var I,J : Integer;
    St : TStringList;
    S : String;
begin
...
Function TGameDBH.GetWWWNameList(WithDefaultProfile : Boolean =True; const HideWindowsProfiles : Boolean = False) : TStringList;
Var I,J : Integer;
    St : TStringList;
    S : String;
begin
  ...
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      If HideWindowsProfiles and IsWindowsExeMode(TGameH(FGameList[I]).ProfileMode) then continue;
      For J:=1 to 9 do begin
        S:=TGameH(FGameList[I]).WWWNamePlain[J];
...
Function TGameDBH.GetKeyValueList(const Key : String; WithDefaultProfile : Boolean =True) : TStringList;
...
begin
  ...
  try
    For I:=0 to FGameList.Count-1 do If WithDefaultProfile or (TGameH(FGameList[I]).Name<>DosBoxDOSProfile) then begin
      ...
      St2:=StringToStringList(TGameH(FGameList[I]).CacheUserInfo);
      try
        Val:='';
...
Function TGameDBH.GetUserKeys : TStringList;
...
begin
  ...
  try
    For I:=0 to FGameList.Count-1 do begin
      St:=StringToStringList(TGameH(FGameList[I]).CacheUserInfo);
      try
...
procedure TGameDBH.GameChanged(Sender: TObject);
begin
  If Assigned(FOnChanged) then FOnChanged(self);
end;

procedure TGameDBH.StoreAllValues;
Var I : Integer;
begin
  For I:=0 to FGameList.Count-1 do TGameH(FGameList[I]).StoreAllValues;
end;

procedure TGameDBH.LoadCache;
Var I : Integer;
begin
...
Var St,StFinal : TMemoryStream;
    ...
    A : AnsiString;
begin
  LogInfo('StoreBinCache: writing '+IntToStr(FGameList.Count)+' entries to '+FDir+CacheFile);
  ...
  try
    ...
    try
      {Write ID}
      StFinal.WriteBuffer(CacheVersionString[1],length(CacheVersionString)*SizeOf(Char));
      StFinal.WriteBuffer(CacheInfoString[1],length(CacheInfoString)*SizeOf(Char));
      If FGameList.Count>0 then I:=TGameH(FGameList[0]).GetBinaryVersionID else I:=0;
      StFinal.WriteBuffer(I,SizeOf(Integer));

      {Write index — on-disk: Word length + that many Ansi bytes (historical format)}
      For C:='A' to 'Z' do begin
        I:=Index[C].Count; StFinal.WriteBuffer(I,SizeOf(Integer));
        For I:=0 to Index[C].Count-1 do begin
          A:=AnsiString(ChangeFileExt(Index[C][I],''));
          W:=length(A); StFinal.WriteBuffer(W,SizeOf(Word));
          If W>0 then StFinal.WriteBuffer(A[1],W);
      ...
      For I:=0 to IndexRest.Count-1 do begin
        A:=AnsiString(ChangeFileExt(IndexRest[I],''));
        W:=length(A); StFinal.WriteBuffer(W,SizeOf(Word));
        If W>0 then StFinal.WriteBuffer(A[1],W);
      ...
      LogInfo('StoreBinCache: saved '+FDir+CacheFile);
    finally
      StFinal.Free;
    end;
  finally
...
Procedure TGameDBH.InitLoadBinCache;
Var C : Char;
    S : String;
    A : AnsiString;
    I,J,K : Integer;
...
begin
  ...
  try
    ...
    BinLoadCache.Position:=BinLoadCache.Position+length(CacheInfoString)*SizeOf(Char);
  ...
  try
    For C:='A' to 'Z' do begin
      ...
      For J:=0 to I-1 do begin
        ...
        SetLength(A,W); If W>0 then BinLoadCache.ReadBuffer(A[1],W);
        BinLoadCache.ReadBuffer(K,SizeOf(Integer));
        BinLoadCacheIndex[C].AddObject(String(A),TObject(K));
      end;
    end;
    BinLoadCache.ReadBuffer(I,SizeOf(Integer));
    For J:=0 to I-1 do begin
...
Procedure TGameDBH.DoneLoadBinCache;
Var C : Char;
begin
  BinLoadCache.Free;
...
Function TGameDBH.GetLoadBinCacheOffset(const FileName : String; const DOSFileDate : Integer) : Boolean;
Var S : String;
    C : Char;
    I,J : Integer;
begin
  ...
  try
    ...
    on E: Exception do begin
      LogInfo('GetLoadBinCacheOffset: failed file="'+FileName+'" '+E.ClassName+': '+E.Message);
      exit;
    end;
  end;
...
begin
  Result := -1;
  S := Trim(ExtUpperCase(InstallNameOrDefault));
  if (S = '') or (S = 'DEFAULT') then
  begin
    if PrgSetup.DOSBoxSettingsCount > 0 then
      Result := 0;
    Exit;
  end;
  for I := 1 to PrgSetup.DOSBoxSettingsCount - 1 do
    if Trim(ExtUpperCase(PrgSetup.DOSBoxSettings[I].Name)) = S then
    begin
      Result := I;
      Exit;
    end;
...
begin
  Result := '';
  Raw := Trim(CustomDOSBoxDir);
  Idx := ResolveDOSBoxInstallIndex(Raw);
  if Idx >= 0 then
  begin
    if Idx >= PrgSetup.DOSBoxSettingsCount then
      Exit;
    Dir := Trim(PrgSetup.DOSBoxSettings[Idx].DosBoxDir);
    if Dir = '' then
      Exit;
    Result := IncludeTrailingPathDelimiter(MakeAbsPath(Dir, PrgSetup.BaseDir));
    Exit;
  end;
  if Raw = '' then
    Exit;
  Result := IncludeTrailingPathDelimiter(MakeAbsPath(Raw, PrgSetup.BaseDir));
end;
...
begin
  ...
  if (Idx < 0) or (Idx >= PrgSetup.DOSBoxSettingsCount) then
    Exit;
  Result := Trim(PrgSetup.DOSBoxSettings[Idx].DosBoxMapperFile);
end;
...
begin
  Result := '';
  S := Trim(CurrentMapperField);
  if (S <> '') and (ExtUpperCase(S) <> 'DEFAULT') then
  begin
    Result := ExtractFilePath(MakeAbsPath(S, PrgSetup.BaseDir));
    if Result <> '' then
      Exit;
  end;
  Mapper := DefaultMapperFileForProfile(CustomDOSBoxDir);
  if Mapper <> '' then
    Result := ExtractFilePath(MakeAbsPath(Mapper, PrgSetup.BaseDir));
end;
...
var
  FileName: String;
begin
  FileName := DOSBoxKindToIconFileName(Kind);
  Result := ProgramInstallDir + IconSetsFolder + '\DOSBoxKind\' + FileName;
  if not FileExists(Result) then
    Result := PrgDataDir + IconSetsFolder + '\DOSBoxKind\' + FileName;
  if not FileExists(Result) then
    Result := '';
end;
...
var
  FileName: String;
begin
  FileName := DOSBoxKindToPreviewFileName(Kind);
  Result := ProgramInstallDir + IconSetsFolder + '\DOSBoxKind\' + FileName;
  if not FileExists(Result) then
    Result := PrgDataDir + IconSetsFolder + '\DOSBoxKind\' + FileName;
  if not FileExists(Result) then
...
initialization
  DefaultValueReaderGame:=TGameH.Create('');
finalization

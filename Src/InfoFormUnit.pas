...
type
  TInfoForm = class(TForm)
    ...
    procedure DFXHomepageLabelClick(Sender: TObject);
...
uses ShellAPI, VistaToolsUnit, LanguageSetupUnit, CommonHelpers, CommonTools, PrgConsts,
...
begin
  ...
  { Product name + version from PE FileVersion (RAD VerInfo).
    Layout: DFendX, Version (tight), DFendX homepage URL, blank line, written-by, email.
    HomepageLabel (Reloaded SF) caption left unchanged. }
  Label1.Caption:='DFendX';
  VersionLabel.Caption:=GetFileVersionAsString;
  VersionLabel.AutoSize:=True;
  DFXHomepageLabel.Caption:=DFRHomepage;
  DFXHomepageLabel.AutoSize:=True;
  { Prefer language string; hard-fallback if lang file still has old short "written by". }
  S:=Trim(LanguageSetup.InfoFormWrittenBy);
  If (S='') or (ExtUpperCase(S)='WRITTEN BY') then
    S:='A fork of D-Fend Reloaded written by';
  WrittenByLabel.Caption:=S+' Alexander Herzog';
  ...
  Label1.Top:=16;
  VersionLabel.Top:=Label1.Top+Label1.Height+2;
  DFXHomepageLabel.Top:=VersionLabel.Top+VersionLabel.Height+2;
  WrittenByLabel.Top:=DFXHomepageLabel.Top+DFXHomepageLabel.Height+32;
  eMailLabel.Top:=WrittenByLabel.Top+WrittenByLabel.Height+4;
  HomepageLabel.Top:=eMailLabel.Top+eMailLabel.Height+4;
  ...
  LicenseComboBox.Items.AddObject('DFendX',TObject(0));
  ...
  S:=LanguageSetup.InfoFormDFend;
  { Hard-fallback if language file still has pre-fork wording only. }
  If (Pos('fork and modernization',ExtLowerCase(S))=0) and (Pos('complete rewrite of the discontinued',ExtLowerCase(S))>0) then
    S:='DFendX is a fork and modernization effort of D-Fend Reloaded.[13][13]'+
       'D-Fend Reloaded is a complete rewrite of the discontinued good old D-Fend.[13][13]'+
       'D-Fend (http:/'+'/members.home.nl/mabus/) was once one of the best DOSBox frondends and I was very sad to see its development was stopped.[13][13]'+
       'On the other hand: If the development of D-Fend had not stopped, DFendX would have never been written.';
...
procedure TInfoForm.DFXHomepageLabelClick(Sender: TObject);
begin
  ShellExecute(Handle,'open',PChar(DFXHomepageLabel.Caption),nil,nil,SW_SHOW);
end;

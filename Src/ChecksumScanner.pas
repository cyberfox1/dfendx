...
uses SysUtils, IniFiles, CommonHelpers, HashCalc, WaitFormUnit, LanguageSetupUnit;
...
begin
  ...
  try
    result:=Ini.ReadString('LanguageFileInfo','DownloadChecksum','');
    if result = '' then
      result := inherited GetFileChecksum(Filename);
  finally
    Ini.Free;
  end;
end;

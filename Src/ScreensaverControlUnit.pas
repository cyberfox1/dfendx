...
uses Windows, CommonHelpers, PrgSetupUnit;
...
procedure TScreensaverControl.InitTimer;
begin
...
begin
  If (DOSBoxHandle=0) or (DOSBoxHandle=INVALID_HANDLE_VALUE) then exit;
  If HandleList.Count=0 then OrigScreensaverStatus:=GetScreensaverAllowStatus;
  If not DuplicateHandle(GetCurrentProcess,DOSBoxHandle,GetCurrentProcess,@NewHandle,0,False,DUPLICATE_SAME_ACCESS) then exit;

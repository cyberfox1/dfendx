...
begin
  ...
  UserIconLoader.AddEmptyImages(ImageList,nil,39);
...
begin
  ...
  F:=TSetupFrameExoDOS.Create(self); AddTreeNode(Root,F,TSetupFrameExoDOS(F),False,26);
...
begin
  ...
  try
    ...
    For I:=0 to length(Frames)-1 do If (not Frames[I].HideInTree) and ((not Frames[I].AdvancedModeOnly) or AdvancedMode) then begin
...
begin
  ...
  {Add pages}
  For I:=0 to length(Frames)-1 do If (not Frames[I].IsEmpty) and (not Frames[I].HideInTree) then begin

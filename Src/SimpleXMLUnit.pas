...
uses XMLDoc, XMLIntf, SysUtils, Forms, Math, CommonHelpers, CommonTools, PrgSetupUnit,
     DOSBoxUnit, GameDBToolsUnit, GameDBToolsHelpers, GameDBHelpers, LanguageSetupUnit;
...
begin
  ...
  St.Add('<RealGenerator>DFendX '+GetNormalFileVersionAsString+'</RealGenerator>');
  St.Add('</Export>');
  ...
  For I:=0 to GamesList.Count-1 do begin
    ...
    St2:=BuildConfFile(G,False,False,-1,nil,false); if St2=nil then continue;
    try If St2.Count>0 then begin St2[0]:='      <![CDATA['+St2[0]; St2.Add(']]>'); end; St.AddStrings(St2); finally St2.Free; end;
    St.Add('      </Full-Configuration>');
...
begin
  ...
  For I:=0 to GamesList.Count-1 do begin
    ...
    try If St2.Count>0 then begin St2[0]:='    <![CDATA['+St2[0]; St2.Add(']]>'); end; St.AddStrings(St2); finally St2.Free; end;
    St.Add('  </full-configuration>');
{  <incremental-configuration> always identical to full-configuration because
    BuildConfFile has no delta mode. D-Fend's import ignores it too. }
    {St.Add('  <incremental-configuration>');
    St2:=BuildConfFile(G,False,False,-1,nil,true); if St2=nil then continue;
    try If St2.Count>0 then begin St2[0]:='    <![CDATA['+St2[0]; St2.Add(']]>'); end; St.AddStrings(St2); finally St2.Free; end;
    St.Add('  </incremental-configuration>');}
    St.Add('  <dosbox>');
...
begin
  ...
  Case XMLType of
    xtDBGL : N:=Profile.ChildNodes.FindNode('title');
    xtDOG : begin
              N:=Profile.ChildNodes.FindNode('Configuration');
              If N<>nil then N:=N.ChildNodes.FindNode('Information');
              If N<>nil then N:=N.ChildNodes.FindNode('Name');
            end;
    else exit;
  end;
...
begin
  ...
  try
    If XMLType=xtDBGL then begin
      N:=Profile.ChildNodes.FindNode('meta-info');
    ...
    If XMLType=xtDOG then begin
      ...
      If N<>nil then For I:=0 to N.ChildNodes.Count-1 do begin
        ...
        If S='YEAR' then begin G.Year:=N2.Text; continue; end;
        If S='GENRE' then begin G.Genre:=N2.Text; continue; end;
        If S='NOTE' then begin G.Notes:=N2.Text; continue; end;
        ...
        If S='CUSTOMINFORMATION' then For J:=0 to N2.ChildNodes.Count-1 do begin N3:=N2.ChildNodes[J]; St.Add(N3.NodeName+'='+N3.Text); end;
      end;
    end;
...
begin
  If XMLType=xtDBGL then begin
    N:=Profile.ChildNodes.FindNode('full-configuration');
  end;

  If XMLType=xtDOG then begin
    N:=Profile.ChildNodes.FindNode('Configuration');
    If N<>nil then N:=N.ChildNodes.FindNode('Settings');
    If N<>nil then N:=N.ChildNodes.FindNode('Full-Configuration');
  end;
...
Var N,N2 : IXMLNode;
    ...
    S : String;
begin
  N:=Profile.ChildNodes.FindNode('dosbox');
  If N=nil then exit;
...
begin
  ...
  For I:=0 to N.ChildNodes.Count-1 do begin
    ...
    If Trim(ExtUpperCase(N2.NodeName))=ExtUpperCase(DataKey) then begin
      Values:=N2.ChildNodes.FindNode('Values');
...
Var N,N2,N3 : IXMLNode;
    S : String;
begin
  N:=Profile.ChildNodes.FindNode('Emulator');
  If N<>nil then begin

    N2:=N.ChildNodes.FindNode('Path');
    If N2<>nil then G.CustomDOSBoxDir:=MakeRelPath(N2.Text,PrgSetup.BaseDir,True);

    N2:=N.ChildNodes.FindNode('Configuration');
    If N2<>nil then begin
      N3:=N2.ChildNodes.FindNode('Glide');
      If N3<>nil then begin
        if N3.Text='0' then G.GlideEmulation:='false' else G.GlideEmulation:='true';
      end;
  ...
  If N<>nil then begin
    ...
    N2:=N.ChildNodes.FindNode('Information');
    If N2<>nil then begin
      N3:=N2.ChildNodes.FindNode('ExeFilename');
      If N3<>nil then G.GameExe:=DOGFileNameToFileName(N3.Text);
      N3:=N2.ChildNodes.FindNode('SetupFilename');
      If N3<>nil then G.SetupExe:=DOGFileNameToFileName(N3.Text);
      N3:=N2.ChildNodes.FindNode('EXECommandLine');
      If N3<>nil then G.GameParameters:=N3.Text;
      N3:=N2.ChildNodes.FindNode('SetupCommandLine');
      If N3<>nil then G.SetupParameters:=N3.Text;
    end;

    N2:=N.ChildNodes.FindNode('Settings');
    If N2<>nil then N2:=N2.ChildNodes.FindNode('Addons');
    If N2<>nil then begin
      N3:=N2.ChildNodes.FindNode('DOS32A');
      If N3<>nil then begin
        N3:=N3.ChildNodes.FindNode('Used');
        If (N3<>nil) and (N3.Text<>'0') then G.UseDOS32A:=True;
      end;
      N3:=N2.ChildNodes.FindNode('i4DOS');
      If N3<>nil then begin
        N3:=N3.ChildNodes.FindNode('Used');
        If (N3<>nil) and (N3.Text<>'0') then G.Use4DOS:=True;
      end;
      N3:=N2.ChildNodes.FindNode('Mouse');
      If N3<>nil then begin
        N3:=N3.ChildNodes.FindNode('AutoLock');
...
begin
  ...
  try
    ...
    If not XMLDoc.Active then begin
      ...
      result:='Could not activate the XML document.';
      exit;
    end;

...
Type TPrgSetup=class(TBasePrgSetup)
  ...
  public
    ...
    Destructor Destroy; override;
    { Call once after Create. Loads DOSBox/packer/interpreter lists.
      Not re-entrant — do not call again on the same instance. }
...
begin
  Result := False;
  if FDosBoxKind <> dbkStaging then
    Exit;
  if Trim(FDosBoxVersion) = '' then begin
    Result := True;
    Exit;
  end;
...
begin
  ...
  If OperationMode=omPortable then begin
    ...
    ExoDOSDir:=MakeExtAbsPath(ExoDOSDir,PrgDir);
    { ExoDOSVersion is a version label (e.g. "6.0"), not a filesystem path. }
...
begin
  If OperationMode=omPortable then begin
    ...
    ExoDOSDir:=MakeExtRelPath(ExoDOSDir,PrgDir);
    { ExoDOSVersion is not a path — leave as stored version string. }
  end;
end;

Function TPrgSetup.HasValidExoInstallation : Boolean;
begin
  result:=(Trim(ExoDOSDir)<>'') and (Trim(ExoDOSVersion)<>'');
end;
...
begin
  If OperationMode=omUserDir then begin
    result:=IncludeTrailingPathDelimiter(GetSpecialFolder(0,CSIDL_PROFILE))+'DFendX\';
  end else begin
    result:=PrgDir;
  end;
end;
...
begin
  ...
  If FileExists(PrgDataDir+SettingsFolder+'\'+FileName) then begin
    SysUtils.DeleteFile(PrgDataDir+FileName);
  end else begin
    If Windows.CopyFile(PChar(PrgDataDir+FileName),PChar(PrgDataDir+SettingsFolder+'\'+FileName),True) then
      SysUtils.DeleteFile(PrgDataDir+FileName)
    else
      MessageDlg('Could not move file '+FileName+' to '+PrgDataDir+SettingsFolder,mtError,[mbOK],0);
  end;
end;

...
uses Windows, SysUtils, Dialogs, Xml.XMLDom, Xml.XMLDoc, Xml.Win.msxmldom, PackageDBToolsUnit,
     CommonHelpers, CommonTools, PrgConsts, PrgSetupUnit, LanguageSetupUnit, GameDBToolsUnit, GameDBToolsHelpers, System.UITypes, XMLHelpers;
...
begin
  result:=False;
  if not NodeHasAttributes(N,['Name',FChecksumXMLName,'Size']) then exit;
  FName:=DecodeHTMLSymbols(N.Attributes['Name'], LanguageSetup.CharsetHTMLTranslate);
...
begin
  N.Attributes['Name']:=EncodeHTMLSymbols(FName, LanguageSetup.CharsetHTMLTranslate);
...
begin
  ...
  if not NodeHasAttributes(N,['Genre','Developer','Publisher','Year','Language']) then exit;
  FGenre:=DecodeHTMLSymbols(N.Attributes['Genre'], LanguageSetup.CharsetHTMLTranslate);
  FDeveloper:=DecodeHTMLSymbols(N.Attributes['Developer'], LanguageSetup.CharsetHTMLTranslate);
  FPublisher:=DecodeHTMLSymbols(N.Attributes['Publisher'], LanguageSetup.CharsetHTMLTranslate);
  FYear:=DecodeHTMLSymbols(N.Attributes['Year'], LanguageSetup.CharsetHTMLTranslate);
  FLanguage:=DecodeHTMLSymbols(N.Attributes['Language'], LanguageSetup.CharsetHTMLTranslate);
...
begin
  ...
  N.Attributes['Genre']:=EncodeHTMLSymbols(FGenre, LanguageSetup.CharsetHTMLTranslate);
  N.Attributes['Developer']:=EncodeHTMLSymbols(FDeveloper, LanguageSetup.CharsetHTMLTranslate);
  N.Attributes['Publisher']:=EncodeHTMLSymbols(FPublisher, LanguageSetup.CharsetHTMLTranslate);
  N.Attributes['Year']:=EncodeHTMLSymbols(FYear, LanguageSetup.CharsetHTMLTranslate);
  N.Attributes['Language']:=EncodeHTMLSymbols(FLanguage, LanguageSetup.CharsetHTMLTranslate);
...
begin
  result:=False;
  if not inherited LoadFromXMLNode(N) then exit;
  if not NodeHasAttributes(N,['License']) then exit;
  FLicense:=DecodeHTMLSymbols(N.Attributes['License'], LanguageSetup.CharsetHTMLTranslate);
...
begin
  ...
  N.Attributes['License']:=EncodeHTMLSymbols(FLicense, LanguageSetup.CharsetHTMLTranslate);
...
begin
  result:=False;
  if not inherited LoadFromXMLNode(N) then exit;
  If not NodeHasAttributes(N,['MinVersion','MaxVersion','Author']) then exit;
  ...
  FAuthor:=DecodeHTMLSymbols(N.Attributes['Author'], LanguageSetup.CharsetHTMLTranslate);
  result:=True;
end;
...
begin
  ...
  N.Attributes['Author']:=EncodeHTMLSymbols(FAuthor, LanguageSetup.CharsetHTMLTranslate);
end;
...
begin
  result:=False;
  If not inherited LoadFromXMLNode(N) then exit;
  if not NodeHasAttributes(N,['Description']) then exit;
  FDescription:=DecodeHTMLSymbols(N.Attributes['Description'], LanguageSetup.CharsetHTMLTranslate);
  result:=True;
end;
...
begin
  inherited SaveToXMLNode(N);
  N.Attributes['Description']:=EncodeHTMLSymbols(FDescription, LanguageSetup.CharsetHTMLTranslate);
end;
...
begin
  ...
  If N.HasAttribute('Name') then FProviderName:=DecodeHTMLSymbols(N.Attributes['Name'], LanguageSetup.CharsetHTMLTranslate);
  If N.HasAttribute('Text') then FProviderText:=DecodeHTMLSymbols(N.Attributes['Text'], LanguageSetup.CharsetHTMLTranslate);
...
begin
  ...
  try
    ...
    FName:=DecodeHTMLSymbols(XMLDoc.DocumentElement.Attributes['Name'], LanguageSetup.CharsetHTMLTranslate);
...
begin
  ...
  try
    try
      Doc.DOMVendor:=Xml.Win.msxmldom.MSXML_DOM;
      Doc.Active:=True;
    except
    ...
    if HasProviderInfo then begin
      ...
      If FProviderName<>'' then Node.Attributes['Name']:=EncodeHTMLSymbols(FProviderName, LanguageSetup.CharsetHTMLTranslate);
      If FProviderText<>'' then Node.Attributes['Text']:=EncodeHTMLSymbols(FProviderText, LanguageSetup.CharsetHTMLTranslate);

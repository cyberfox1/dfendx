...
type
  ...
  TGIFVersionRec = GIFImageReader.TGIFVersionRec;
...
begin
  ...
  begin
    ...
    if (ErrorCode <> 0) and (FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, nil,
      ...
      raise EOutOfResources.Create(string(Buf)) at ReturnAddr
    else
...
type
  // Use the one from GIFImageReader to avoid duplicate type
  TGIFHeaderRec = GIFImageReader.TGIFHeaderRec;
...
begin
  ...
  GifHeader.Signature[2] := 'F';  // will be converted to AnsiChar
...
begin
  ...
  // Delegate signature validation to the pure GIFImageReader unit
  if not IsValidGIFSignature(GifHeader) then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      NewKey := (KeyInt(Prefix) SHL 8) OR Color;
...
{$ENDIF}
    ...
    try (* Protect above resources*)
      ...
      begin
        ...
        MappedTransparentIndex := Color.Blue DIV 51 +
      ...
      try
        ...
        begin
          ...
          begin
            ...
            FillChar(Dst^, Width, MappedTransparentIndex);
...
{$endif}
  ...
  begin
    ...
    try
      for i := 0 to Height-1 do
      begin
        MaskPixel := PByte(DIBReader.Scanline[i]);
        GIFPixel := PByte(Scanline[i]);
...
begin
  ...
  begin
    pSource := PByte(NativeUInt(Previous.Scanline[PreviousY]) + Cardinal(MergeRect.Left) - Cardinal(Previous.Left));
    pDest := PByte(NativeUInt(Scanline[Y]) + Cardinal(MergeRect.Left) - Cardinal(Left));
    ...
    begin
      ...
      if (
            not(
              ((not NeedTransparentColorIndex) and (pDest^ = DestIndex)) or
              ((SourceTransparent) and (pSource^ = SourceIndex))
            )
          ) and (
            (* Replace same colored pixels with transparency*)
            ((pDestMap = pSourceMap) and (pDest^ = pSource^)) or
            (CompareMem(@(pDestMap^[pDest^]), @(pSourceMap^[pSource^]), sizeof(TGIFColor)))
          ) then
      begin
...
begin
  Result := string(AnsiString(PAnsiChar(@FIdent.Authentication[0])));
end;
...
begin
  S := AnsiString(Value);
  if (Length(S) < sizeof(TGIFAuthenticationCode)) then
    FillChar(FIdent.Authentication, sizeof(TGIFAuthenticationCode), 32);
  StrLCopy(PAnsiChar(@FIdent.Authentication[0]), PAnsiChar(S), sizeof(TGIFAuthenticationCode));
end;

function TGIFApplicationExtension.GetIdentifier: string;
begin
  Result := string(AnsiString(PAnsiChar(@FIdent.Identifier[0])));
end;
...
begin
  S := AnsiString(Value);
  if (Length(S) < sizeof(TGIFIdentifierCode)) then
    FillChar(FIdent.Identifier, sizeof(TGIFIdentifierCode), 32);
  StrLCopy(PAnsiChar(@FIdent.Identifier[0]), PAnsiChar(S), sizeof(TGIFIdentifierCode));
end;
...
begin
  ...
  begin
    ...
    Priority := tpNormal;
    if (Suspended) then
      inherited Start; (* Must be running before we can terminate*)
  end;
end;
...
begin
  ...
  *)
  ...
  begin
    ...
    if (not IsTransparent) or (Pixel^ <> OldTransparentColorIndex) then
    begin
      (* Check for invalid color index*)
      if (Pixel^ >= FCount) then
      begin
  ...
  for i := 0 to FImages.Count-1 do
    ...
    begin
      ...
      begin
        ...
        if (IsTransparent) and (Pixel^ = OldTransparentColorIndex) then
...
var
  ...
  function Scan(Buf: Pointer; Value: Byte; Count: integer): boolean; assembler;
  asm
    PUSH	EDI
    MOV		EDI, Buf
    MOV		ECX, Count
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            if not(Scan(Pointer(Images[i].Data),
...
begin
  with FPainters.LockList do
    try
      for i := 0 to Count-1 do
        TGIFPainter(Items[i]).Suspend;  // deprecated but kept for compatibility with old painter logic
    finally
      FPainters.UnLockList;
    end;
end;

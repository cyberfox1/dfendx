...
type
  ...
  private
    ...
    Procedure RefreshPixelShaderList;
...
uses Spin, Math, LanguageSetupUnit, CommonHelpers, CommonTools, VistaToolsUnit, IconLoaderUnit,
     GameDBToolsUnit, GameDBToolsHelpers, GameDBHelpers, HelpConsts, PrgSetupUnit, PrgConsts, System.Types,
     MultipleProfilesEditorFormHelpers;
...
begin
  ...
  { Filled by RefreshPixelShaderList from checked profiles' Kind + ResolveDOSBoxDir }
  St:=TStringList.Create;
  try
    AddSetting(5401,LanguageSetup.GamePixelShader,St,False,-1,False);
  finally
    St.Free;
  end;
  ...
  If IsValueInList(GameDB.ConfOpt.MIDIDevice,'mt32') then begin
...
procedure TMultipleProfilesEditorForm.ListBoxClick(Sender: TObject);
begin
  RefreshPixelShaderList;
end;

procedure TMultipleProfilesEditorForm.RefreshPixelShaderList;
Var I,J : Integer;
    CheckBox : TCheckBox;
    Combo : TComboBox;
    G : TGame;
...
begin
  ...
  For I:=0 to length(Sections)-1 do
    For J:=0 to length(Sections[I].Settings)-1 do
      If Sections[I].Settings[J].ID=5401 then begin
        CheckBox:=Sections[I].Settings[J].CheckBox;
        If length(Sections[I].Settings[J].Values)>0 then
          Combo:=TComboBox(Sections[I].Settings[J].Values[0].Control);
        Found:=True;
        break;
      end;
  If (not Found) or (CheckBox=nil) or (Combo=nil) then exit;

  First:=True;
  Kind:=dbkNone;
  Dir:='';
  For I:=0 to ListBox.Count-1 do If ListBox.Checked[I] then begin
  ...
  St:=GetPixelShaders(Dir,Kind);
  try
    Combo.Items.BeginUpdate;
    try
      Combo.Items.Clear;
      Combo.Items.AddStrings(St);
      If Combo.Items.Count>0 then Combo.ItemIndex:=0 else Combo.ItemIndex:=-1;
    finally

...
uses SysUtils, IniFiles, PackageDBToolsUnit, CommonHelpers, CommonTools, LanguageSetupUnit,
     PrgSetupUnit, PrgConsts, PackageManagerHelpers;
          ...
          For J:=0 to K-1 do begin
            ...
            If HideIfAlreadyInstalled then begin
              If AutoSetups then begin
                ...
                If FindGameByNameOrChecksum(Data.Name,Data.GameExeChecksum,AutoSetupDB) then continue;
                If Trim(Data.MaxVersion)<>'' then begin
                  If VersionToInt(Data.MaxVersion)<Ver then continue;
                end;
              end else begin
                If FindGameByNameOrChecksum(Data.Name,Data.GameExeChecksum,GameDB) then continue;
              end;
            end;
            {Filter}
            Case Filter.Key of
  ...
  try
    For I:=0 to PackageDB.Count-1 do begin
      ...
      For J:=0 to K-1 do begin
        ...
        If not AutoSetups then AddUniqueSemicolonValues(TDownloadZipData(Data).License,FilterSelectLicenseUpper,FilterSelectLicense);
        AddUniqueSemicolonValues(Data.Genre,FilterSelectGenreUpper,FilterSelectGenre);
        AddUniqueSemicolonValues(Data.Developer,FilterSelectDeveloperUpper,FilterSelectDeveloper);
        AddUniqueSemicolonValues(Data.Publisher,FilterSelectPublisherUpper,FilterSelectPublisher);
        AddUniqueSemicolonValues(Data.Year,FilterSelectYearUpper,FilterSelectYear);
        AddUniqueSemicolonValues(Data.Language,FilterSelectLanguageUpper,FilterSelectLanguage);
      end;
    end;

...
uses Classes, Forms, XMLIntf, CheatDBHelpers;
...
Type TCheatActionStepChangeAddress=class(TCheatActionStep, IMergeableActionStep)
  ...
  private
    ...
    { IInterface — always implemented (Delphi + FPC). Only the IID param
      passing differs: FPC IInterface uses constref; Delphi uses const. }
{$IFDEF FPC}
...
uses Windows, SysUtils, Xml.XMLDoc, Xml.Win.msxmldom, Dialogs, Math, CheatDBToolsUnit, PrgConsts,
     CheatDBInternalUnit, PackageDBToolsUnit, LanguageSetupUnit, System.UITypes;
...
{$IFDEF FPC}
function TCheatActionStepChangeAddress.QueryInterface(constref IID: TGUID; out Obj): HResult; stdcall;
{$ELSE}
function TCheatActionStepChangeAddress.QueryInterface(const IID: TGUID; out Obj): HResult; stdcall;
{$ENDIF}
begin
  if GetInterface(IID, Obj) then Result := S_OK else Result := E_NOINTERFACE;
end;

function TCheatActionStepChangeAddress._AddRef: Integer; stdcall;
begin
  Result := -1; { non-refcounted: lifetime owned by TCheatAction / list }
end;

function TCheatActionStepChangeAddress._Release: Integer; stdcall;
begin
  Result := -1;
end;

function TCheatActionStepChangeAddress.GetBytes: Integer;
begin
  Result := FBytes;
end;

function TCheatActionStepChangeAddress.GetNewValue: string;
begin
  Result := FNewValue;
end;

function TCheatActionStepChangeAddress.GetAddresses: string;
begin
  Result := FAddresses;
end;

procedure TCheatActionStepChangeAddress.SetAddresses(const Value: string);
begin
  SetString(0, Value);
end;
...
begin
  result:=CheatDBHelpers.Apply(St,FAddressesInt,FBytes,FNewValueInt);
end;
...
begin
  ...
  result:=CheatDBHelpers.Apply(St,FAddressesInt,FBytes,I);
end;
...
begin
  ...
  { Per-action snapshot so a multi-step failure can roll back without undoing
    other cheats already applied to the same file in this session. }
  TempBackup:=FileName+'.applytmp';
  If not CopyFile(PChar(FileName),PChar(TempBackup),False) then begin
    result:=False;
    exit;
  end;
  try
    ...
    If not result then
      CopyFile(PChar(TempBackup),PChar(FileName),False);
  finally
...
begin
  CheatDBHelpers.CompressActionSteps(FActionSteps);
end;
...
begin
  ...
  try
    try
      XMLDoc.DOMVendor:=Xml.Win.msxmldom.MSXML_DOM;
      XMLDoc.Active:=True;
    except

...
uses Classes, FileNameConvertor;
...
uses SysUtils, Dialogs, Math, DOSBoxUnit, CommonHelpers, PrgConsts, PrgSetupUnit, LanguageSetupUnit, System.UITypes, ExoDOSHelpers;
...
Function PathNameOK(SubDir : String) : Boolean;
{ 8.3 + DosBoxShortNameAllowedChars only (see PrgConsts). }
Var I : Integer;
    Name, Ext : String;
begin
  result:=False;
  ...
  For I:=1 to length(Name) do if Pos(Name[I],DosBoxShortNameAllowedChars)<0 then exit;
  For I:=1 to length(Ext) do if Pos(Ext[I],DosBoxShortNameAllowedChars)<0 then exit;
...
begin
  ...
  If GameExeIsUnderExoDOS(IncludeTrailingPathDelimiter(RootPath)+LongName,PrgSetup.ExoDOSDir) then exit;

...
uses Windows, SysUtils, Math, CommonHelpers, PrgConsts, PrgSetupUnit,
     DOSBoxTempUnit, DOSBoxUnit, MainUnit, AnsiStrings;

const SpecialChars : Array[0..6] of AnsiChar = (#27, #$DA, #$C4, #$B3, #$BF, #$C0, #$D9);
...
begin
  ...
  Src:=AnsiString(result);
  SetLength(Dst,Length(Src));
  If Length(Src)>0 then
    CharToOemA(PAnsiChar(Src),PAnsiChar(Dst));
  SetLength(Dst,AnsiStrings.StrLen(PAnsiChar(Dst)));
  result:=String(Dst);

  For I:=Low(SpecialChars) to High(SpecialChars) do result:=Replace(result,SpecialSymbols[I],String(SpecialChars[I]));
end;
...
begin
  ...
  For I:=Low(SpecialChars) to High(SpecialChars) do result:=Replace(result,String(SpecialChars[I]),SpecialSymbols[I]);

  Src:=AnsiString(result);
  SetLength(Dst,Length(Src));
  If Length(Src)>0 then
    OEMToCharA(PAnsiChar(Src),PAnsiChar(Dst));
  SetLength(Dst,AnsiStrings.StrLen(PAnsiChar(Dst)));
  result:=String(Dst);
end;
...
begin
  ...
  try
    while I=0 do begin
      ...
      For J:=0 to St.Count-1 do If Trim(ExtUpperCase(St[J]))=S then begin B:=False; break; end;

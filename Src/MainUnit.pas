...
type
  TDFendReloadedMainForm = class(TForm, IDropTarget)
    ...
    MenuProfileAddFromExoDOS: TMenuItem;
    ...
    MenuHelpDosBoxIntroDosBoxX: TMenuItem;
    MenuHelpDosBoxIntroStaging: TMenuItem;
    N13: TMenuItem;
    N14: TMenuItem;
    N16: TMenuItem;
    ...
    AddButtonMenuAddFromExoDOS: TMenuItem;
    N17: TMenuItem;
    ...
    ExoExtraTab: TTabSheet;
    ExoExtraListView: TListView;
    DocTab: TTabSheet;
    DocListView: TListView;
    ...
    ToolButtonSoundtrackSep: TToolButton;
    SoundtrackAutoplaySwitch: TToggleSwitch;
  ...
  private
    ...
    Procedure ScrollGameNotesToTop;
...
uses ShellAPI, ShlObj, ClipBrd, Math, CommCtrl, CommonHelpers, CommonTools, LanguageSetupUnit,
     PrgConsts, VistaToolsUnit, PrgSetupUnit, SetupDosBoxFormUnit,
     SetupFormUnit, IconManagerFormUnit, DosBoxUnit,
     ...
     CreateShortcutsFormUnit, PackageManagerFormUnit, PackageBuilderUnit,
      RunPrgManagerUnit, ScreensaverControlUnit, PackageCreationFormUnit, TextEditPopupUnit,
...
begin
  ...
  ScreensaverControl.InitTimer;
  RunPrgManager.InitTimer;
  ZipManager.InitTimer;
  ...
  ScreenshotsCache:=nil;
  CacheDir:=IncludeTrailingPathDelimiter(PrgDataDir+SettingsFolder);
  try
    { FireDAC silent before first connect — avoids vendor-lib dialog retry spam. }
    EnsureFireDACSilent;
    ScreenshotsCache:=TScreenshotsCache.Create(CacheDir+ScreenshotsCacheDBFileName);
    LogInfo('Screenshots cache OK path="'+CacheDir+ScreenshotsCacheDBFileName+'"');
  except
    on E: Exception do begin
      ScreenshotsCache:=nil;
      LogInfo('Screenshots cache FAILED path="'+CacheDir+ScreenshotsCacheDBFileName+'" err="'+E.Message+'"');
      { Single UI report; cache stays disabled for this session (nil). }
      MessageDlg(
        'Screenshots thumbnail cache could not be opened (disabled for this session).'+#13#10#13#10+
        E.Message+#13#10#13#10+
        'Path: '+CacheDir+ScreenshotsCacheDBFileName,
        mtError,[mbOK],0);
    end;
  end;
  ...
  If PrgSetup.FirstRun then begin
    LogInfo('First run: Searching DOSBox (standard + X + Staging)');
    if SearchDosBoxAll(self,PrgSetup.DOSBoxSettings[0].DosBoxDir,DOSBoxSearch) then
      ApplyDOSBoxSearchResult(DOSBoxSearch);
  ...
  if not InitBassMedia(PrgDir+BinFolder, Handle) then
    LogInfo('WARNING: BASS init failed – internal sound player may be unavailable')
  else
    BindBassSoundLoad;
  AfterPlaySoundDialog:=UpdateAmbientSoundtrack;
...
begin
  ...
  PrgSetup.SoundtrackAutoplay := False;
  ...
  If PrgSetup.FirstRun then begin
    ...
    If CurrentINIVersion<>CurrentExeVersion then begin
      ...
      LogInfo('Upgrade: versions match, skipping');
...
begin
  ...
  MenuHelpDosBoxIntroDosBoxX.Caption:=LanguageSetup.MenuHelpDOSBoxIntroDOSBoxX;
  MenuHelpDosBoxIntroStaging.Caption:=LanguageSetup.MenuHelpDOSBoxIntroStaging;
  ...
  ExoExtraTab.Caption:=LanguageSetup.ExtrasTab;
  ...
  ExoExtraImageList.Height:=ScreenshotImageList.Height;
  ExoExtraImageList.Width:=ScreenshotImageList.Width;
  DocImageList.Height:=16;
  DocImageList.Width:=16;
  DocListView.SmallImages:=DocImageList;
...
begin
  B:=False;
  { Prefer program-folder CHM (ships with the build) over data-dir copies that may be stale. }
  HelpFileName:=PrgDir+LanguageSubDir+'\'+LanguageSetup.HelpFile; If FileExists(HelpFileName) then B:=True;
  If not B then begin HelpFileName:=PrgDataDir+LanguageSubDir+'\'+LanguageSetup.HelpFile; If FileExists(HelpFileName) then B:=True; end;
  ...
  If B then begin
    ...
    If HTMLhelpRouter=nil then HTMLhelpRouter:=InitHTMLHelp(HelpFileName) else begin
      HTMLhelpRouter.HelpType:=htHTMLhelp;
      HTMLhelpRouter.Helpfile:=HelpFileName;
    end;
  end else begin
    { No CHM on disk: still install the HTML-help router so F1 never falls
      through to WinHelp (which opens the Windows 10 end-of-life webpage). }
    If HTMLhelpRouter=nil then HTMLhelpRouter:=InitHTMLHelp('');
  end;
end;
...
begin
  ...
  with ExoExtraListView do begin ViewStyle:=vsReport; ViewStyle:=vsIcon; end;
  ...
  { T8: window title (Application.Title set in DFend.dpr). Safe after removing
    idle-handler Free of GameDB when Caption lacked "D-Fend". }
  if Trim(Application.Title) <> '' then
...
Function TDFendReloadedMainForm.FirstSoundtrackPath : String;
Var CaptureDir, P : String;
    I : Integer;
begin
  result:='';
  If (ListView.Selected=nil) or (ListView.Selected.Data=nil) then exit;
  CaptureDir:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
...
Procedure TDFendReloadedMainForm.UpdateAmbientSoundtrack;
Var P : String;
begin
  If not PrgSetup.SoundtrackAutoplay then begin
  ...
  If FPostShowGateEnabled then exit;
  If (DOSBoxCounter.Count>0) or (ScummVMCounter.Count>0) or (WindowsGameCounter.Count>0) then exit;
  P:=FirstSoundtrackPath;
  If P='' then begin
    PauseAmbientSoundtrack;
    exit;
  end;
  PlayAmbientSoundtrack(P);
end;

Procedure TDFendReloadedMainForm.ScrollMediaListToTop(const AListView : TListView);
Var Origin : TPoint;
begin
  if (AListView=nil) or (not AListView.HandleAllocated) or (AListView.Items.Count=0) then exit;

  { Selection of a later item keeps the view pinned; clear it on profile refresh. }
  AListView.Selected:=nil;
  AListView.ItemFocused:=nil;

  { Reset scroll to content origin (0,0). EnsureVisible(last) is scroll-to-bottom —
    never use that for "start at top". }
  if ListView_GetOrigin(AListView.Handle,Origin) then
    ListView_Scroll(AListView.Handle,-Origin.X,-Origin.Y);
...
Var S, ExtraRootPath : String;
    MediaDB : TExoDOSMediaDB;
    GamesDB : TExoDOSDB;
    Game : TGame;
begin
  MediaDB:=TExoDOSMediaDB.Create(PrgDataDir+ExoDOSMediaDBFile);
  try
  ...
  { ----- Screenshots tab -----
    Source A: CaptureFolder (non-eXo screenshots from game profile)
    Source B: MediaDB Images category, Kind LIKE 'Screenshot -%' }
  ...
  try
    ...
    If Game<>nil then begin
      ...
      If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
      If S<>'' then AddScreenshotsToList(ScreenshotListView,ScreenshotImageList,S); {####### NON EXO SCREENSHOTS #######}
      AddExoScreenshotsToList(MediaDB,ScreenshotListView,ScreenshotImageList,Game,ExtraRootPath); { ####### EXO SCREENSHOTS #######}
    end;
  finally
  ...
  { ----- Sounds tab -----
    ...
    Source B: MediaDB Music category }
  ...
  try
    ...
    If Game<>nil then begin
      S:=Trim(Game.CaptureFolder);
      If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
      If S<>'' then AddSoundsToList(SoundListView,S,mliSound); {####### NON EXO SOUNDS #######}
      AddExoSoundsToList(MediaDB,SoundListView,Game,mliSound,ExtraRootPath); { ####### EXO SOUNDS #######}
    end;
  finally
  ...
  { ----- Videos tab -----
    ...
    Source B: MediaDB Videos category }
  ...
  try
    ...
    If Game<>nil then begin
      S:=Trim(Game.CaptureFolder);
      If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
      If S<>'' then AddVideosToList(VideoListView,S,mliVideo); {####### NON EXO VIDEOS #######}
      AddExoVideosToList(MediaDB,VideoListView,Game,mliVideo,ExtraRootPath); { ####### EXO VIDEOS #######}
    end;
  finally
  ...
  { ----- Extras tab -----
    ...
    Source B: ExoDOS game root Extras folder (ExoRoot + RootFolder + \Extras\) }
  ExoExtraListView.Items.BeginUpdate;
  try
    ExoExtraListView.Items.Clear;
    ExoExtraImageList.Clear;
    If PrgSetup.HasValidExoInstallation and (Game<>nil) then
      AddExoExtrasToList(MediaDB,ExoExtraListView,ExoExtraImageList,Game,ExtraRootPath); {######## NON-SCREENSHOT IMAGES AND EXTRAS FOLDER IMAGES #######}
  finally
    ExoExtraListView.Items.EndUpdate;
  end;
  ...
  { ----- Docs tab -----
    ...
    Source B: ExoDOS game root Extras folder, document extensions }
  DocListView.Items.BeginUpdate;
  try
    DocListView.Items.Clear;
    If PrgSetup.HasValidExoInstallation and (Game<>nil) then
      AddExoDocsToList(MediaDB,DocListView,DocImageList,Game,ExtraRootPath);
  finally
...
Function TDFendReloadedMainForm.ImageMediaListView : TListView;
begin
  { Shared ScreenshotPopupMenu: PopupComponent is the list that was right-clicked (or set by DblClick/KeyDown). }
  If ScreenshotPopupMenu.PopupComponent=ExoExtraListView then
    result:=ExoExtraListView
  else
    result:=ScreenshotListView;
end;

Procedure TDFendReloadedMainForm.ScrollGameNotesToTop;
begin
  if (GameNotesEdit=nil) or (not GameNotesEdit.HandleAllocated) then exit;
  if not GameNotesEdit.Visible then exit;
  GameNotesEdit.SelStart:=0;
  GameNotesEdit.SelLength:=0;
  SendMessage(GameNotesEdit.Handle,EM_SETSEL,0,0);
  SendMessage(GameNotesEdit.Handle,WM_VSCROLL,SB_TOP,0);
  SendMessage(GameNotesEdit.Handle,EM_SCROLLCARET,0,0);
  { EM_LINESCROLL second param: lines to scroll; large negative forces top. }
  SendMessage(GameNotesEdit.Handle,EM_LINESCROLL,0,LPARAM(-MaxInt div 2));
end;
...
begin
  ...
  St2:=TStringList.Create;
  try
    For I:=0 to St.Count-1 do If St[I]<>'' then begin
      If St2.Count>0 then St2.Add('');
      St2.Add(St[I]);
    end;
    { Assign leaves caret at end; force viewport to top (SelStart alone is not enough). }
    GameNotesEdit.Lines.BeginUpdate;
    try
      GameNotesEdit.Lines.Assign(St2);
    finally
      GameNotesEdit.Lines.EndUpdate;
    end;
    ScrollGameNotesToTop;
  finally St2.Free; St.Free; end;
end;
...
Var TemplateDB : TGameDB;
    ...
    M : TMenuItem;
begin
  LogInfo('UpdateAddFromTemplateMenu: called');
...
Procedure TDFendReloadedMainForm.AddFromExoDOS;
Var SL, SL2 : TStringList;
    Form : TListScummVMGamesForm;
    DB : TExoDOSDB;
    Title, Year, DataBlob, RootFolder, ConfigPath, ConfPath : String;
    Lines : TStringList;
    GameResult : TExoDOSAutoexecResult;
    TemplateDB : TGameDB;
    MountAbs, MountRel, MountStr, SetupRel, BootImg, S, GameRoot : String;
...
begin
  ...
  try
    ...
    if SL.Count=0 then begin
      MessageDlg(LanguageSetup.MessageExoDOSNoGamesList,mtError,[mbOK],0);
      exit;
    end;

    Form:=TListScummVMGamesForm.Create(self,SL);
    try
      Form.Selectable:=True;
      Form.OKButton.Visible:=True;
      Form.Caption:=LanguageSetup.MessageExoDOSSelectGame;
      if Form.ShowModal=mrOK then begin
        if Form.SelectedGame<>'' then begin
          Title:=StripExoTrailingYear(Form.SelectedGame);
          Year:='';
          DataBlob:='';
          DB:=TExoDOSDB.Create(ExoDOSGamesList.GetDBPath);
          try
            if DB.GetGameDataByName(Title,Year,DataBlob) then begin
              RootFolder:=ParseDataBlobValue(DataBlob,'RootFolder');
              if RootFolder='' then begin
                MessageDlg(Format(LanguageSetup.MessageExoDOSNoRootFolder,[Title]),mtError,[mbOK],0);
                exit;
              end;
              GameRoot:=IncludeTrailingPathDelimiter(PrgSetup.ExoDOSDir)+'eXo\eXoDOS\'
                +ExtractFileName(ExcludeTrailingPathDelimiter(RootFolder))+'\';
              ConfPath:=IncludeTrailingPathDelimiter(PrgSetup.ExoDOSDir)+RootFolder+'\dosbox.conf';
              if not FileExists(ConfPath) then begin
                MessageDlg(Format(LanguageSetup.MessageExoDOSConfNotFound,[ConfPath]),mtError,[mbOK],0);
                exit;
              end;
              Lines:=GetAutoExecSection(ConfPath);
              try
                GameResult:=GetGameExe(Lines);
                if GameResult=nil then begin
                  MessageDlg(Format(LanguageSetup.MessageExoDOSNoGameExe,[ConfPath]),mtError,[mbOK],0);
                  exit;
                end;
                try
                  ConfigPath:=ParseDataBlobValue(DataBlob,'ConfigurationPath');
                  MountAbs:=IncludeTrailingPathDelimiter(PrgSetup.ExoDOSDir)+'eXo\eXoDOS\';
                  MountRel:=MakeRelPath(MountAbs,PrgSetup.BaseDir);
                  MountStr:=MountRel+';DRIVE;C;False;;';
                  { Prefer setup already on disk if the game is unpacked; else eXo install.bat. }
                  S:=FindExoUnpackedSetupProgram(GameRoot);
                  if S<>'' then
                    SetupRel:=S
                  else if ConfigPath<>'' then
                    SetupRel:=IncludeTrailingPathDelimiter(PrgSetup.ExoDOSDir)+ConfigPath
                  else
                    SetupRel:='';
                  if SetupRel<>'' then
                  ...
                  try
                    ...
                    for I:=0 to TemplateDB.Count-1 do
                      if SameText(TemplateDB[I].Name,'Normal DOS Games') then begin
                        TemplateFound:=True;
                        break;
                      end;

                    NewIndex:=GameDB.Add(Title);
                    NewGame:=GameDB[NewIndex];
                    ...
                    NewGame.CloseDosBoxAfterGameExit:=True;
                    NewGame.SetupExe:=SetupRel;

                    if GameResult.BootCommand<>'' then begin
                      S:=Trim(Copy(GameResult.BootCommand,5,MaxInt));
                      I:=Pos(' ',S);
                      if I>0 then BootImg:=Trim(Copy(S,1,I-1)) else BootImg:=Trim(S);
                      NewGame.GameExe:='';
                      NewGame.AutoexecBootImage:=MakeRelPath(GameRoot+BootImg,PrgSetup.BaseDir);
                    end else begin
                      NewGame.GameExe:=MakeRelPath(GameRoot+GameResult.ExeName,PrgSetup.BaseDir);
                      NewGame.GameParameters:=GameResult.Params;
                    end;

                    { After template + exe path: eXo profiles should open windowed with aspect. }
                    NewGame.StartFullscreen:=False;
                    NewGame.AspectCorrection:=True;

                    NewGame.Genre:=ParseDataBlobValue(DataBlob,'Genre');
                    NewGame.Developer:=ParseDataBlobValue(DataBlob,'Developer');
                    NewGame.Publisher:=ParseDataBlobValue(DataBlob,'Publisher');
                    NewGame.Year:=Year;
                    NewGame.Notes:=ParseDataBlobValue(DataBlob,'Notes');
              ...
              MessageDlg(Format(LanguageSetup.MessageExoDOSGameNotInDB,[Title]),mtError,[mbOK],0);
          finally
            DB.Free;
          end;
        end;
  ...
  try
    Case (Sender as TComponent).Tag of
      ...
      3002 : begin
               ...
               {For fixing problems with not hidden tooltips} ListView.ShowHint:=False; ListView.ShowHint:=PrgSetup.ShowTooltips;
                G:=TGame(ListView.Selected.Data);
                S:=MakeAbsPath(G.SetupExe,PrgSetup.BaseDir);
                if (Pos('!dos',S)>0) and SameText(ExtractFileName(S),'install.bat') then
                  ShellExecute(Handle,'open',PChar(S),nil,PChar(ExtractFilePath(S)),SW_SHOW)
                else if WindowsExeMode(G) then RunWindowsGame(G,True) else RunGame(G,DeleteOnExit,True);
             end;
      ...
      6030 : ShellExecute(Handle,'open',PChar('https:/'+'/dosbox-x.com/wiki/Home'),nil,nil,SW_SHOW);
      6031 : ShellExecute(Handle,'open',PChar('https:/'+'/www.dosbox-staging.org/0.83/getting-started/introduction/'),nil,nil,SW_SHOW);
...
begin
  If Sender=ExoExtraListView then
    ...
    ScreenshotPopupMenu.PopupComponent:=ScreenshotListView;
  LV:=ImageMediaListView;
  ExtrasMode:=(LV=ExoExtraListView);
  B:=(Item<>nil);
  ...
  If ExtrasMode then begin
    ScreenshotPopupImport.Enabled:=False;
    ScreenshotPopupRenameAll.Enabled:=False;
    ScreenshotPopupDeleteAll.Enabled:=False;
  end else If (ListView.Selected<>nil) and (ListView.Selected.Data<>nil) then begin
    S:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
    If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
...
begin
  If (Sender=ExoExtraListView) or (Sender=ScreenshotListView) then
    ScreenshotPopupMenu.PopupComponent:=TComponent(Sender);
  ScreenshotMenuWork(ScreenshotPopupOpen);
end;

procedure TDFendReloadedMainForm.SoundListViewDblClick(Sender: TObject);
begin
  LogInfo('SoundListViewDblClick: selected='+
    BoolToStr(SoundListView.Selected<>nil,True)+
    ' game='+BoolToStr((ListView.Selected<>nil) and (ListView.Selected.Data<>nil),True));
...
procedure TDFendReloadedMainForm.DocListViewDblClick(Sender: TObject);
begin
  if DocListView.Selected<>nil then
    ShellExecute(0,'open',PChar(DocListView.Selected.SubItems[0]),nil,nil,SW_SHOWNORMAL);
end;

procedure TDFendReloadedMainForm.DocListViewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Shift=[]) and (Key=VK_RETURN) and (DocListView.Selected<>nil) then
...
begin
  If (Sender=ExoExtraListView) or (Sender=ScreenshotListView) then
    ScreenshotPopupMenu.PopupComponent:=TComponent(Sender);
  ExtrasMode:=(ImageMediaListView=ExoExtraListView);
  If (not ExtrasMode) and (Shift=[ssShift]) and (Key=VK_DELETE) then begin ScreenshotMenuWork(ScreenshotPopupDeleteAll); exit; end;
  ...
  Case Key of
    ...
    VK_INSERT : If not ExtrasMode then ScreenshotMenuWork(ScreenshotPopupImport);
    VK_F5 : ScreenshotMenuWork(ScreenshotPopupRefresh);
    VK_DELETE : If not ExtrasMode then ScreenshotMenuWork(ScreenshotPopupDelete);
  end;
end;
...
begin
  ...
  Case (Sender as TComponent).Tag of
    0 : If LV.Selected<>nil then begin
          If (ListView.Selected=nil) or (ListView.Selected.Data=nil) then exit;
          S:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
          If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
    ...
    3 : If LV.Selected<>nil then begin
          If (ListView.Selected=nil) or (ListView.Selected.Data=nil) then exit;
          S:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
          If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
          T:=GetImageListItemFilePath(LV.Selected,S);
          ...
          SaveImageToFile(T,ScreenshotSaveDialog.FileName);
        end;
    4 : begin
          ...
          If LV.Selected<>nil then begin
            ...
            If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
            If S='' then exit;
            if not ExtDeleteFile(IncludeTrailingPathDelimiter(S)+LV.Selected.Caption,ftMediaViewer) then
              Messagedlg(Format(LanguageSetup.MessageCouldNotDeleteDir,[IncludeTrailingPathDelimiter(S)+LV.Selected.Caption]),mtError,[mbOK],0);
            ListViewSelectItem(Sender,ListView.Selected,True);
          end;
        end;
    5 : begin
          ...
          If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
          If S='' then exit;
          For I:=0 to LV.Items.Count-1 do if not ExtDeleteFile(IncludeTrailingPathDelimiter(S)+LV.Items[I].Caption,ftMediaViewer) then begin
            MessageDlg(Format(LanguageSetup.MessageCouldNotDeleteDir,[IncludeTrailingPathDelimiter(S)+LV.Items[I].Caption]),mtError,[mbOK],0);
            break;
          end;
          ListViewSelectItem(Sender,ListView.Selected,True);
        end;
    ...
    7 : If LV.Selected<>nil then begin
          If (ListView.Selected=nil) or (ListView.Selected.Data=nil) then exit;
          S:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
          If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
          T:=GetImageListItemFilePath(LV.Selected,S);
    ...
    8 : begin
          ...
          If LV.Selected<>nil then begin
            ...
            T:=ChangeFileExt(LV.Selected.Caption,'');
            If not InputQuery(LanguageSetup.ScreenshotPopupRenameCaption,LanguageSetup.ScreenshotPopupRenameLabel,T) then exit;
            T:=T+ExtractFileExt(LV.Selected.Caption);
            If not RenameFile(S+LV.Selected.Caption,S+T) then
              MessageDlg(Format(LanguageSetup.MessageCouldNotRenameFile,[S+LV.Selected.Caption,S+T]),mtError,[mbOK],0);
            ListViewSelectItem(Sender,ListView.Selected,True);
            T:=Trim(ExtUpperCase(T));
            For I:=0 to LV.Items.Count-1 do
              If Trim(ExtUpperCase(LV.Items[I].Caption))=T then begin
    ...
    10: If LV.Selected<>nil then begin
          ...
          If (S<>'') and (S<>'INTERNAL') then begin
            If PrgSetup.NonModalViewer
              then ShowNonModalImageDialog(self,T,nil,nil)
              else ShowImageDialog(self,T,nil,nil);
            exit;
          end;
...
begin
  Case (Sender as TComponent).Tag of
    0 : begin
          if SoundListView.Selected<>nil then
            LogInfo('SoundMenuWork Open: tag=0 caption="'+SoundListView.Selected.Caption+'"')
          else
            LogInfo('SoundMenuWork Open: tag=0 caption=""');
          If SoundListView.Selected=nil then begin
            LogInfo('SoundMenuWork Open: abort no list selection');
            exit;
          end;
          If (ListView.Selected=nil) or (ListView.Selected.Data=nil) then begin
            LogInfo('SoundMenuWork Open: abort no game selection');
            exit;
          end;
          S:=Trim(TGame(ListView.Selected.Data).CaptureFolder);
          If S<>'' then S:=MakeAbsPath(S,PrgSetup.BaseDir);
          ...
          If (T='') or (not FileExists(T)) then begin
            LogInfo('SoundMenuWork Open: abort empty path or file missing');
            exit;
          end;
          St1:=TStringList.Create;
          St2:=TStringList.Create;
          try
            ...
            LogInfo('SoundMenuWork Open: PlaySoundDialog returned');
          finally
            St1.Free;
            St2.Free;
          end;
    ...
    2 : If SoundListView.Selected<>nil then begin
          ...
          T:=Trim(ExtUpperCase(PrgSetup.SoundPlayer));
          If (T<>'') and (T<>'INTERNAL') then begin
            PlaySoundDialog(self,S,nil,nil);
            exit;
          end;
...
begin
  ...
  GamesRunning:=(DOSBoxCounter.Count>0) or (ScummVMCounter.Count>0) or (WindowsGameCounter.Count>0);
  If FGamesWereRunning and (not GamesRunning) then
    UpdateAmbientSoundtrack;
  FGamesWereRunning:=GamesRunning;
...
begin
  ...
  If CapturePageControl.ActivePageIndex=GameNotesPanel.PageIndex then begin
    If ListView.Selected<>nil then begin
      ...
      { Scroll stuck if Assign ran while tab was inactive; re-apply on show. }
      ScrollGameNotesToTop;
    end else begin
      GameNotesEdit.Visible:=True;
...
Procedure TDFendReloadedMainForm.SyncSoundtrackAutoplaySwitch;
Var OldClick : TNotifyEvent;
    Cap : String;
begin
  If SoundtrackAutoplaySwitch=nil then exit;
  SoundtrackAutoplaySwitch.Hint:=LanguageSetup.ButtonAutoplayHintMain;
  { Caption lives on the switch (interposer TToggleSwitch); only the glyph is clickable. }
  Cap:=LanguageSetup.ButtonAutoplay;
  SoundtrackAutoplaySwitch.ShowStateCaption:=True;
  SoundtrackAutoplaySwitch.StateCaptions.CaptionOn:=Cap;
  SoundtrackAutoplaySwitch.StateCaptions.CaptionOff:=Cap;
  OldClick:=SoundtrackAutoplaySwitch.OnClick;
  SoundtrackAutoplaySwitch.OnClick:=nil;
  try
    If PrgSetup.SoundtrackAutoplay then begin
      SoundtrackAutoplaySwitch.ThumbColor:=TColor($00228B22);
      SoundtrackAutoplaySwitch.State:=tssOn;
    end else begin
      SoundtrackAutoplaySwitch.State:=tssOff;
      SoundtrackAutoplaySwitch.ThumbColor:=clGray;
    end;
  finally
    SoundtrackAutoplaySwitch.OnClick:=OldClick;
  end;
end;

procedure TDFendReloadedMainForm.SoundtrackAutoplaySwitchClick(Sender: TObject);
begin
  PrgSetup.SoundtrackAutoplay:=(SoundtrackAutoplaySwitch.State=tssOn);
  If SoundtrackAutoplaySwitch.State=tssOn then
    SoundtrackAutoplaySwitch.ThumbColor:=TColor($00228B22)
  else
    SoundtrackAutoplaySwitch.ThumbColor:=clGray;
  UpdateAmbientSoundtrack;

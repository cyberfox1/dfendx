...
uses Windows, Classes, ComCtrls, Controls, Graphics, Menus, CheckLst,
     GameDBUnit, GameDBHelpers, LinkFileUnit, PrgConsts, ExoDOSDBUnit;
...
Procedure AddExoVideosToList(const MediaDB : TExoDOSMediaDB; const AListView : TListView; const AGame : TGame; ImageListIndex : TMediaListImageIndex; const ExtraRootPath : String);
Procedure AddExoDocsToList(const MediaDB : TExoDOSMediaDB; const AListView : TListView; const AImageList : TImageList; const AGame : TGame; const ExtraRootPath : String);
...
{Get Pixel shader list (ids for combo/INI; Kind selects root + extensions)}

Function GetPixelShaders(const DOSBoxDir : String; const Kind : TDOSBoxKind) : TStringList;

{ Embedded multi-size ICON resources. Bitmaps are rasterized at a fixed size for TImage; caller must not Free. Nil if load fails. }
function GetDOSBoxKindBitmap(Kind: TDOSBoxKind): TBitmap;
function GetGenDocBitmap: TBitmap;
function GetTooBigPlaceholderBitmap: TBitmap;
{ Load embedded ICON at the given pixel size (uses multi-size ICO frames). Caller must Free. }
function LoadEmbeddedIconResource(const ResName: String; const W, H: Integer): TIcon;
{ DOSBox-kind ICON at W×H (nearest multi-size frame). Caller must Free. }
function LoadDOSBoxKindIcon(Kind: TDOSBoxKind; const W, H: Integer): TIcon;

implementation

uses SysUtils, GameDBToolsHelpers, Forms, Dialogs, ShellAPI, ShlObj, IniFiles, Math, Vcl.Imaging.pngimage,
...
var
  { Per-list-refresh cache: one image-list slot per DOSBox kind (reused by many profiles).
    FKindIconIndex values are -1 until a kind icon is added in the current list build. }
  ...
  FTooBigThumbTried: Boolean = False;
  FDosBoxKindBmp: array[TDOSBoxKind] of TBitmap;
...
{ LoadEmbeddedIconResource is declared in the interface. }

{ AddDosBoxKindIconsToList is defined after ScaleGraphicToImageList. }
...
function AddDosBoxKindIconsToList(const Kind: TDOSBoxKind;
  ...
  const BackgroundColor: TColor): Integer;
{ Returns image index in AListViewIconImageList, or -1 if no icon loaded.
  Uses multi-size ICON resources — no runtime bitmap scale. }
var
  Ico: TIcon;
  ResName: String;
begin
  Result := -1;
  if (AListViewIconImageList = nil) or (AListViewImageList = nil) then Exit;

  if not FKindIconCacheInited then
    InvalidateDosBoxKindListIconCache;

  if (FKindIconList = AListViewIconImageList) and (FKindIconIndex[Kind] >= 0) then
  begin
    Result := FKindIconIndex[Kind];
    Exit;
  end;

  ResName := DOSBoxKindToResourceName(Kind);

  Ico := LoadEmbeddedIconResource(ResName, AListViewImageList.Width, AListViewImageList.Height);
  if Ico = nil then Exit;
  try
    AListViewImageList.AddIcon(Ico);
  finally
    Ico.Free;
  end;

  Ico := LoadEmbeddedIconResource(ResName, AListViewIconImageList.Width, AListViewIconImageList.Height);
  if Ico = nil then Exit;
  try
    AListViewIconImageList.AddIcon(Ico);
  finally
    Ico.Free;
  end;

  Result := AListViewIconImageList.Count - 1;
  FKindIconList := AListViewIconImageList;
...
begin
  ...
  { DOSBox-mode profiles with no custom icon: default list icon by DOSBox kind. }
...
begin
  ...
  St:=TStringList.Create;
  try
    CollectMedia(Dir, ImageExtensions, St, 0, 1);
...
Var St : TStringList;
  ...
  procedure AppendEntry(const FullPath, Caption : String);
  begin
    N:=Length(Entries);
    SetLength(Entries,N+1);
    Entries[N].FullPath:=FullPath;
    Entries[N].Size:=GetFileSize(FullPath);
    Entries[N].Caption:=Caption;
  end;
  ...
  begin
    ...
    try
      for I:=0 to St.Count-1 do begin
        ...
        if Cap='' then Cap:=ExtractFileName(St[I]);
        AppendEntry(St[I], Cap);
      end;
    finally
      St.Free;
    end;
  ...
  procedure CollectGameRootExtras;
  Var I : Integer;
  begin
    if Trim(ExtraRootPath)='' then exit;
    St:=GetExoGameExtrasPaths(PrgSetup.ExoDOSDir, ExtraRootPath, ImageExtensions);
    try
      for I:=0 to St.Count-1 do begin
        Cap:=ChangeFileExt(ExtractFileName(St[I]), '');
        if Cap='' then Cap:=ExtractFileName(St[I]);
        AppendEntry(St[I], Cap);
      end;
    finally
  ...
  begin
    for I:=0 to High(Entries) do begin
      IsLarge:=(Entries[I].Size>MaxThumbSourceBytes);
      ...
      if IsLarge then begin
        ...
        B:=ScreenshotsCache.GetThumbnail(Entries[I].FullPath,AImageList.Width,AImageList.Height);
        if B=nil then continue;
        ImgIdx:=AImageList.AddMasked(B,clNone);
        B.Free;
      end;

      L:=AListView.Items.Add;
      { Caption always from source (platform type folder / Extras filename), even if thumb is placeholder. }
      L.Caption:=Entries[I].Caption;
      L.ImageIndex:=ImgIdx;
      L.SubItems.Add(Entries[I].FullPath);
    end;
  end;
...
begin
  ...
  try
    for K:=0 to St.Count-1 do begin
      if St.Count=1 then
        ...
        Cap:='Soundtrack #'+IntToStr(K+1)+' (Exo)';

      L:=AListView.Items.Add;
      L.Caption:=Cap;
      L.ImageIndex:=Ord(ImageListIndex);
      L.SubItems.Add(St[K]);
    end;
  finally
    St.Free;
  ...
  { Game RootFolder\Extras\ audio — not "Soundtrack" so ambient ignores them. }
  if Trim(ExtraRootPath)='' then exit;
  St:=GetExoGameExtrasPaths(PrgSetup.ExoDOSDir, ExtraRootPath, AudioExtensions);
  try
    for K:=0 to St.Count-1 do begin
      Cap:=ChangeFileExt(ExtractFileName(St[K]), '');
      if Cap='' then Cap:=ExtractFileName(St[K]);
      Cap:=Cap+' (Extras)';
...
begin
  ...
  if Trim(ExtraRootPath)='' then exit;
  St:=GetExoGameExtrasPaths(PrgSetup.ExoDOSDir, ExtraRootPath, VideoExtensions);
  try
    for K:=0 to St.Count-1 do begin
      Cap:=ChangeFileExt(ExtractFileName(St[K]), '');
      if Cap='' then Cap:=ExtractFileName(St[K]);
...
Procedure AddExoDocsToList(const MediaDB : TExoDOSMediaDB; const AListView : TListView; const AImageList : TImageList; const AGame : TGame; const ExtraRootPath : String);
Var St, ExtMap : TStringList;
    K, IconIdx : Integer;
    L : TListItem;
    FilePath, Ext : String;
    SFI : TSHFileInfo;
    Ico : TIcon;
    FallbackIconIdx : Integer;
begin
  if (AGame=nil) or (not PrgSetup.HasValidExoInstallation) then exit;
  ...
  try
    ...
    try
      ...
      for K:=0 to St.Count-1 do begin
        ...
        IconIdx:=ExtMap.IndexOf(Ext);
        if IconIdx<0 then begin
          FillChar(SFI,SizeOf(SFI),0);
          if SHGetFileInfo(PChar(FilePath),0,SFI,SizeOf(SFI),SHGFI_ICON or SHGFI_SMALLICON)<>0 then begin
            if SFI.hIcon<>0 then begin
              Ico:=TIcon.Create;
              try
                Ico.Handle:=SFI.hIcon;
                IconIdx:=AImageList.AddIcon(Ico);
                ExtMap.AddObject(Ext,TObject(IconIdx));
              finally
                Ico.Free;
              end;
          ...
          if IconIdx<0 then begin
            if FallbackIconIdx<0 then begin
              Ico:=LoadEmbeddedIconResource('GENDOC',AImageList.Width,AImageList.Height);
              if Ico<>nil then
              try
                FallbackIconIdx:=AImageList.AddIcon(Ico);
              finally
                Ico.Free;
              end;
            end;
            if FallbackIconIdx<0 then continue;
            IconIdx:=FallbackIconIdx;
            ExtMap.AddObject(Ext,TObject(IconIdx));
          end;
        end else
          IconIdx:=Integer(ExtMap.Objects[IconIdx]);

        L:=AListView.Items.Add;
        L.Caption:=ExtractFileName(FilePath);
        L.ImageIndex:=IconIdx;
...
begin
  ...
  St:=TStringList.Create;
  try
    CollectMedia(Dir, AudioExtensions, St, 0, 1);
    for K:=0 to St.Count-1 do begin
      L:=AListView.Items.Add;
      L.Caption:=ExtractFileName(St[K]);
      L.ImageIndex:=Ord(ImageListIndex);
...
begin
  ...
  St:=TStringList.Create;
  try
    CollectMedia(Dir, VideoExtensions, St, 0, 1);
    for K:=0 to St.Count-1 do begin
      L:=AListView.Items.Add;
      L.Caption:=ExtractFileName(St[K]);
      L.ImageIndex:=Ord(ImageListIndex);
...
begin
  ...
  If WWWEncode and (Column<>glecWWW) then
    result:=EncodeHTMLSymbols(result, LanguageSetup.CharsetHTMLTranslate);
end;
...
begin
  ...
  try
    ...
    St.Add('<html>');
    St.Add('  <head>');
    St.Add('    <title>DFendX</title>');
    St.Add('    <meta http-equiv="content-type" content="text/html; charset='+LanguageSetup.CharsetHTMLCharset+'">');
    St.Add('    <meta name="description" content="DFendX games list">');
    St.Add('    <meta name="DC.creator" content="DFendX '+GetNormalFileVersionAsString+'">');
    ...
    St.Add('  </head>');
    St.Add('  <body>');
    St.Add('    <h1>'+EncodeHTMLSymbols(LanguageSetup.MenuFileExportGamesListHTMLTitle, LanguageSetup.CharsetHTMLTranslate)+'</h1>');
    St.Add('    <table>');
    St.Add('      <tr>');
    For I:=0 to ColumnNames.Count-1 do If (I=0) or (TGamesListExportColumn(I) in Columns) then St.Add(Format('        <th>%s</th>',[EncodeHTMLSymbols(ColumnNames[I], LanguageSetup.CharsetHTMLTranslate)]));
    If glecUserFields in Columns then For I:=0 to UserList.Count-1 do St.Add(Format('        <th>%s</th>',[EncodeHTMLSymbols(UserList[I], LanguageSetup.CharsetHTMLTranslate)]));
    If glecNotes in Columns then St.Add(Format('        <th>%s</th>',[EncodeHTMLSymbols(LanguageSetup.GameNotes, LanguageSetup.CharsetHTMLTranslate)]));
    St.Add('      </tr>');
    ...
    try
      For I:=0 to L.Count-1 do begin
        St.Add('      <tr>');
        St.Add(Format('        <td>%s</td>',[EncodeHTMLSymbols(TGame(L[I]).CacheName, LanguageSetup.CharsetHTMLTranslate)]));
        ...
        If glecUserFields in Columns then begin
          ...
          try
            For J:=0 to UserList.Count-1 do begin
              ...
              St.Add(Format('        <td>%s</td>',[EncodeHTMLSymbols(S, LanguageSetup.CharsetHTMLTranslate)]));
            end;
          finally
            St2.Free;
          end;
        ...
        If glecNotes in Columns then begin
          ...
          try
            For J:=0 to St2.Count-1 do begin
              If J>0 then T:=T+'<br>';
              T:=T+EncodeHTMLSymbols(St2[J], LanguageSetup.CharsetHTMLTranslate);
            end;
          finally
            St2.Free;
          end;
...
begin
  ...
  try
    ...
    St.Add('<!DOCTYPE GamesList SYSTEM "https:/'+'/github.com/cyberfox1/dfendx/GamesListExport/GamesListExport.dtd">');
    St.Add('');
    ...
    try
      For I:=0 to L.Count-1 do begin
        ...
        St.Add(Format('    <Name>%s</Name>',[EncodeHTMLSymbols(TGame(L[I]).CacheName, LanguageSetup.CharsetHTMLTranslate)]));

        For J:=1 to ColumnNames.Count-1 do If TGamesListExportColumn(J) in Columns then begin
          S:=GetGamesListExportColumnValue(TGame(L[I]),TGamesListExportColumn(J),False);
          If TGamesListExportColumn(J)<>glecWWW then S:=EncodeHTMLSymbols(S, LanguageSetup.CharsetHTMLTranslate);
        ...
        If glecUserFields in Columns then begin
          ...
          try
            For J:=0 to UserList.Count-1 do begin
              ...
              S:=EncodeHTMLSymbols(S, LanguageSetup.CharsetHTMLTranslate);
              if Trim(S)<>'' then begin
                St.Add('    <UserData>');
                St.Add(Format('      <Name>%s</Name>',[EncodeHTMLSymbols(UserList[J], LanguageSetup.CharsetHTMLTranslate)]));
        ...
        If glecNotes in Columns then begin
          ...
          St2:=StringToStringList(TGame(L[I]).Notes);
          try
            For J:=0 to St2.Count-1 do T:=T+EncodeHTMLSymbols(St2[J], LanguageSetup.CharsetHTMLTranslate)+#13;
          finally
            St2.Free;
          end;
...
begin
  result:=True;
    If GameExeIsUnderExoDOS(MakeAbsPath(AGame.GameExe,PrgSetup.BaseDir),PrgSetup.ExoDOSDir) then exit;
  ...
  try
    For I:=0 to GameDB.Count-1 do begin
      ...
      If ScummVMMode(Game) then begin
        ...
        If not GameExeIsUnderExoDOS(MakeAbsPath(Game.GameExe,PrgSetup.BaseDir),PrgSetup.ExoDOSDir) then begin
...
Function GetDOSBoxNr(const Game : TGame) : Integer;
begin
  result:=ResolveDOSBoxInstallIndex(Game.CustomDOSBoxDir);
end;
...
begin
  ...
  If ScummVMMode(Game) then begin
    ...
    try
      St.Insert(0,'# This '+T+' configuration file was automatically created by DFendX.');
      St.Insert(1,'# Changes made to this file will NOT be transfered to DFendX profiles list.');
      St.Insert(2,'# DFendX will delete this file from temp directory on program close.');
      St.Insert(3,'');
      ...
      If ScummVMMode(Game) then begin
        ...
        I:=GetDOSBoxNr(Game);
        If I<0 then I:=0;
        T:=GetDOSBoxCommandLine(I,S,Game.ShowConsoleWindow,'');
      end;
...
Function PixelShaderExtAllowed(const Ext : String; const Kind : TDOSBoxKind) : Boolean;
Var E : String;
begin
  E:=ExtUpperCase(Ext);
  if Kind=dbkStaging then
    Result:=(E='.GLSL') or (E='.GSL')
  else
    Result:=(E='.FX');
end;

Function GetPixelShaders(const DOSBoxDir : String; const Kind : TDOSBoxKind) : TStringList;
Var Dir, Root, CatName, CatPath, Id: String;
    Rec, Rec2: TSearchRec;
    I, J: Integer;
    Cats: TStringList;
begin
  result:=TStringList.Create;
  Dir:=IncludeTrailingPathDelimiter(DOSBoxDir);

  case Kind of
    dbkX:       Root:=Dir+'shaders\';
    dbkStaging: Root:=Dir+'resources\'+DosBoxStagingShadersDirLegacy+'\';
  else
    Root:=Dir+'Shaders\';
  end;
  ...
  try
    { Single pass: collect base files and category directory names }
      ...
      while I=0 do begin
        if (Rec.Name<>'.') and (Rec.Name<>'..') then begin
          if (Rec.Attr and faDirectory)<>0 then
            Cats.Add(Rec.Name)
          else if PixelShaderExtAllowed(ExtractFileExt(Rec.Name),Kind) then
            { Base: Root\name.ext → "name" }
    ...
    { One category level: Root\category\name.ext → "category-name" }
      ...
      try
        while J=0 do begin
          if (Rec2.Name<>'.') and (Rec2.Name<>'..') and
             ...
             PixelShaderExtAllowed(ExtractFileExt(Rec2.Name),Kind) then begin
...
function DOSBoxKindToResourceName(Kind: TDOSBoxKind): String;
begin
  case Kind of
...
function LoadEmbeddedIconResource(const ResName: String; const W, H: Integer): TIcon;
{ Multi-size .ico blob as RT_RCDATA (not RT_GROUP_ICON — those pollute PE icon 0).
  SetSize selects the matching frame; no ScaleImage. }
var
  RS: TResourceStream;
begin
  Result := TIcon.Create;
...
function IconResourceToBitmap(const ResName: String; const W, H: Integer): TBitmap;
var
  Ico: TIcon;
begin
  Result := nil;
  Ico := LoadEmbeddedIconResource(ResName, W, H);
  if Ico = nil then Exit;
  try
    Result := TBitmap.Create;
    try
      Result.SetSize(Ico.Width, Ico.Height);
...
function LoadDOSBoxKindIcon(Kind: TDOSBoxKind; const W, H: Integer): TIcon;
begin
  Result := LoadEmbeddedIconResource(DOSBoxKindToResourceName(Kind), W, H);
end;

function GetDOSBoxKindBitmap(Kind: TDOSBoxKind): TBitmap;
{ Cached 256px raster for callers that want a bitmap. List path uses AddIcon. }
begin
  Result := FDosBoxKindBmp[Kind];
  if FDosBoxKindTried[Kind] then
    Exit;
  FDosBoxKindTried[Kind] := True;
  FDosBoxKindBmp[Kind] := IconResourceToBitmap(DOSBoxKindToResourceName(Kind), 256, 256);
  Result := FDosBoxKindBmp[Kind];
end;

function GetGenDocBitmap: TBitmap;
{ Cached 16x16 raster for callers that still want a bitmap. Docs list uses AddIcon. }
begin
  Result := FGenDocBmp;
  if FGenDocTried then
    Exit;
  FGenDocTried := True;
  FGenDocBmp := IconResourceToBitmap('GENDOC', 16, 16);
  Result := FGenDocBmp;
end;
...
begin
  Result := FTooBigThumb;
  if FTooBigThumbTried then
    Exit;
  FTooBigThumbTried := True;
  FTooBigThumb := TBitmap.Create;
  try
    FTooBigThumb.LoadFromResourceName(HInstance, 'TOOBIGTHUMB');
  except
    FreeAndNil(FTooBigThumb);
  end;
  Result := FTooBigThumb;
end;

procedure FreeEmbeddedIconBitmaps;
var
  K: TDOSBoxKind;
begin
  FreeAndNil(FTooBigThumb);
  FreeAndNil(FGenDocBmp);
  for K := Low(TDOSBoxKind) to High(TDOSBoxKind) do
    FreeAndNil(FDosBoxKindBmp[K]);
end;

initialization
  FTooBigThumb := nil;
  FTooBigThumbTried := False;
  FGenDocBmp := nil;
  FGenDocTried := False;

finalization
  FreeEmbeddedIconBitmaps;

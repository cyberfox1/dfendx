...
uses VistaToolsUnit, CommonHelpers, CommonTools, LanguageSetupUnit, PrgSetupUnit, DOSBoxUnit,
     DOSBoxTempUnit, ZipInfoFormUnit, PrgConsts, GameDBToolsUnit, MainUnit, Winapi.Windows, System.UITypes;
...
begin
  ...
  J:=0;
  I:=SysUtils.FindFirst(IncludeTrailingPathDelimiter(Dir)+'*.*',faDirectory,Rec);
  try
    While I=0 do begin
...
begin
  result:='';
  J:=0;
  I:=SysUtils.FindFirst(IncludeTrailingPathDelimiter(NewDir)+'*.*',faDirectory,Rec);
  try
    While I=0 do begin
      If ((Rec.Attr and faDirectory)<>0) and (Rec.Name<>'.') and (Rec.Name<>'..') then begin
        inc(J); If J=10 then Application.ProcessMessages;
...
begin
  ...
  try
    For I:=Low(ProgramExts) to High(ProgramExts) do begin
      J:=SysUtils.FindFirst(S+'*.'+ProgramExts[I],faAnyFile,Rec);
      try
        While J=0 do begin
          St.Add(Rec.Name);
          J:=SysUtils.FindNext(Rec);
        end;
      finally
        SysUtils.FindClose(Rec);
      end;
...
begin
  ...
  For C:='A' to 'Z' do if GetDriveType(PChar(C+':\'))=DRIVE_CDROM then begin
    I:=SysUtils.FindFirst(C+':\*.*',faAnyFile,Rec); SysUtils.FindClose(Rec);

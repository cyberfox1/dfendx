...
type
  ...
  public
    ...
    function Lookup(const Key: String; var FileSize: Int64; var FileDate: TDateTime): Boolean;
    procedure Store(const Key: String; FileSize: Int64; FileDate: TDateTime; const Data: TMemoryStream);
...
uses
  Windows, FireDACSilentUnit, LoggingUnit;
...
begin
  ...
  { Older DBs created without CreatedAt: add column if missing. }
  try
    FConn.ExecSQL(
      'ALTER TABLE Thumbnails ADD COLUMN CreatedAt REAL DEFAULT (julianday(''now''))'
    );
  except
    on E: Exception do
      LogInfo('ScreenshotsDB: ALTER CreatedAt skipped/failed: '+E.ClassName+': '+E.Message);
  end;
end;
...
function TScreenshotsDB.Lookup(const Key: String; var FileSize: Int64; var FileDate: TDateTime): Boolean;
var
  Q: TFDQuery;
begin
  Result := False;
  ...
  try
    Q.Connection := FConn;
    Q.SQL.Text := 'SELECT FileSize, FileDate FROM Thumbnails WHERE Key = :K';
...
procedure TScreenshotsDB.Store(const Key: String; FileSize: Int64; FileDate: TDateTime; const Data: TMemoryStream);
var
  Attempt: Integer;
  Q: TFDQuery;
begin
  ...
  for Attempt := 1 to cStoreMaxAttempts do begin
    ...
    try
      try
        Q.Connection := FConn;
        Q.SQL.Text :=
          'INSERT OR REPLACE INTO Thumbnails (Key, FileSize, FileDate, ImageData, CreatedAt) ' +
          'VALUES (:K, :FS, :FD, :ID, julianday(''now''))';
        Q.ParamByName('K').AsString := Key;

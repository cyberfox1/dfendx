...
//remove drive / Include drive if wanted
      if sz[ 2 ] = ':' then
        begin
         if CharInSet( sz[ 1 ], [ 'A'..'Z','a'..'z' ] ) then
                 ...
                 if ( attr.uiVal and ( 1 shl 4 ) ) <> 0 then
                  begin
                   if isUnicode then
                     ...
                     SysUtils.ForceDirectories(String(sz));
                  end
                 else
                  begin
                    ...
                    if ( not fe ) or ( fe and DoOverwrite ) then begin
                      if isUnicode then
                        ...
                        SysUtils.ForceDirectories(extractfilepath( String( sz ) ) );
                    try
...
begin
  ...
  try
    ...
    //ErikGG 08.11.06
    // Result := true;//Only reached if no error happend  -- removed to avoid overwriting and unused assignment hint
  finally
...
// Shadow 27.11.2006                                           //FHO 17.01.2007
  ...
  if outA.QueryInterface( IID_ISetProperties, setProperties ) = S_OK then begin
    ...
    // set options
    setProperties.SetProperties( @SetPNames, @Setp, NumOfProps );
...
//          j := InternalGetIndexByFilename( Ffiles.WStrings[ i ] );	//ZSA 21.02.2007
          ...
          FMaxProgress := FMaxprogress + Int64(size.uhVal.QuadPart);
          filesDW[ n ] := j;
          Inc( n );
        end; // For i := 0

...
{$WARN SYMBOL_DEPRECATED OFF}  // for FileAge etc in port to modern Delphi
...
begin
  ...
  if not ((Length(Drive) > 1) and (Drive[2] = ':') and
    CharInSet(Upcase(Drive[1]), ['A'..'Z'])) then
    exit;
...
begin
  ...
  begin
    // OEM <-> ANSI conversion (for old zip filename/codepage handling).
    // Always use the *A versions with AnsiString because these are 8-bit codepage conversions.
    var AnsiSrc: AnsiString := AnsiString(Source);
    SetLength(buf, Length(AnsiSrc) + 1);
    if (Direction = cpdOEM2ISO) then
      OemToCharA(PAnsiChar(AnsiSrc), PAnsiChar(buf))
    else
      CharToOemA(PAnsiChar(AnsiSrc), PAnsiChar(buf));
    Result := string(PAnsiChar(buf));
  end;
end;
...
{    case FNumbering of
    ...
    begin
      var DT: TDateTime;
      if FileAge(DiskFile, DT) and (DateTimeToFileDate(DT) = FDateStamp) and (Pred(DiskSeq) < FDiskNr) then
      begin

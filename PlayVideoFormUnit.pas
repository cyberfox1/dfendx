...
uses ShellAPI, Math, MediaInterface, VistaToolsUnit, LanguageSetupUnit,
     CommonHelpers, CommonTools, PrgSetupUnit, PrgConsts, IconLoaderUnit,
     BassMedia, System.UITypes;
...
Var Paused : Boolean;
    ...
    Ok : Boolean;
begin
  { Always open externally. Pause ambient; resume only if the open call fails. }
  Paused:=PauseAmbientSoundtrack;
  Ok:=OpenMediaFile(PrgSetup.VideoPlayer,AFileName);
  if not Ok then begin
    R:=ShellExecute(Application.MainForm.Handle,'open',PChar(AFileName),nil,PChar(ExtractFilePath(AFileName)),SW_SHOW);
    Ok:=(NativeUInt(R)>32);
  end;
  if (not Ok) and Paused then

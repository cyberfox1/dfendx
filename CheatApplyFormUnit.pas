...
uses
  ...
  Dialogs, StdCtrls, CheckLst, Buttons, CheatDBUnit, CheatDBToolsUnit, CheatDBHelpers;
...
uses PrgConsts, CommonHelpers, CommonTools, PrgSetupUnit, VistaToolsUnit, LanguageSetupUnit,
     IconLoaderUnit, HelpConsts, System.UITypes;
          ...
          While K=0 do begin
            If (Rec.Attr and faDirectory)=0 then begin
              ...
              If FilesListBox.Items.IndexOf(S)<0 then begin
                ...
                If DateTimeToFileDate(Rec.TimeStamp)>MostRecentTime then begin MostRecentTime:=DateTimeToFileDate(Rec.TimeStamp); MostRecentNr:=FilesListBox.Items.Count-1; end;
              end;
            end;
            K:=FindNext(Rec);
          end;
...
Var I : Integer;
    A : TCheatAction;
    FileName : String;
    Backuped, Skipped, Failed : TStringList;
    B : Boolean;
...
begin
  ...
  try
    For I:=0 to CheatsListBox.Items.Count-1 do If CheatsListBox.Checked[I] then begin
      ...
      If not FileMatchingMask(ExtractFileName(FileName),A.FileMask) then begin
        Skipped.Add(A.Name);
        continue;
      end;
    ...
    If Skipped.Count>0 then
      MessageDlg(LanguageSetup.ApplyCheatSkippedMaskHeader+#13#10+Skipped.Text,mtWarning,[mbOK],0);

    If Failed.Count>0 then
      MessageDlg(LanguageSetup.ApplyCheatApplyFailed+#13#10+Failed.Text,mtError,[mbOK],0);

    If AppliedCount>0 then
      MessageDlg(LanguageSetup.ApplyCheatApplySuccess,mtInformation,[mbOK],0);
  finally
    Backuped.Free;
    Skipped.Free;
    Failed.Free;

...
uses Windows, SysUtils, Forms, ShellAPI, ActiveX, CommonHelpers, CommonTools,
     DataReaderToolsUnit, PrgConsts, PrgSetupUnit, InternetDataWaitFormUnit, HTTPDownloadHelpers,
     LoggingUnit;
...
begin
  ...
  If (not FileExists(AConfigFile)) and (not DownloadConfigFile(AConfigFile)) then begin
    LogInfo('Moby.LoadConfig: file missing and download failed path='+AConfigFile);
    exit;
  end;
  ...
  repeat
    ...
    If not result then begin
      FreeAndNil(FConfig);
      LogInfo('Moby.LoadConfig: parse failed (ConfigOK=false) path='+AConfigFile);
      If not DownloadConfigFile(AConfigFile) then begin
        LogInfo('Moby.LoadConfig: re-download after parse fail failed path='+AConfigFile);
        exit;
      end;
    end;
  until result;
    ...
    if FLastUpdateCheckOK then begin
      try
        ...
        If I>FConfig.Version then begin
          ...
          result:=FConfig.ConfigOK;
          if not result then begin
            LogInfo('Moby.LoadConfig: parse failed after remote replace path='+AConfigFile);
            exit;
          end;
        end;
      finally
      ...
      LogInfo('Moby.LoadConfig: update download failed, keeping local path='+AConfigFile);
  end;

  FBrowserURLDOS:=FConfig.GamesListURL;
  FBrowserURLAll:=FConfig.GamesListAllPlatformsURL;
  FDataDomain:=THTTPDownloadHelper.DomainOnly(FConfig.GameRecordBaseURL);
end;
...
begin
  ...
  if not result then
    LogInfo('Moby.DownloadConfigFile failed URL='+DataReaderUpdateURL+' Dest='+DestFile);
end;
...
function TMobyDataReader.GetGameData(const Nr: Integer; out Meta: TGameMetadata): Boolean;
Var S,T,Lines, Genre, Developer, Publisher, Year, Notes, ImagePageURL : String;
begin
  result:=False;
  Meta:=nil;
  ...
  Meta:=TGameMetadata.Create(Genre,Developer,Publisher,Year,Notes,ImagePageURL);
  result:=True;
end;
...
begin
  ...
  if not FSuccess then begin
    LogInfo('Moby.LoadConfigThread: LoadConfig failed path='+ConfigPath);
    FSuccess:=FDataReader.LoadConfig(ConfigPath,false);
    if not FSuccess then
      LogInfo('Moby.LoadConfigThread: retry LoadConfig failed path='+ConfigPath);
  end;

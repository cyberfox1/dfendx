...
uses Math, VistaToolsUnit, LanguageSetupUnit, CommonHelpers, PrgSetupUnit,
     ProfileMountEditorFormUnit, GameDBToolsUnit, IconLoaderUnit, System.UITypes,
...
{ NextFreeDriveLetter and UsedDriveLetters extracted to ModernProfileEditorDrivesFrameHelpers }
...
begin
  Case (Sender as TComponent).Tag of
    0 : If Mounting.Count<10 then begin
          S:='';
          if not ShowProfileMountEditorDialog(self,S,UsedDriveLetters(Mounting),IncludeTrailingPathDelimiter(ExtractFilePath(CurrentGameFile)),CurrentGameName,GameDB,nil,NextFreeDriveLetter(Mounting)) then exit;
          Mounting.Add(S);
          LoadMountingList;
    ...
    1 : begin
          ...
          If I<0 then exit;
          S:=Mounting[I];
          if not ShowProfileMountEditorDialog(self,S,UsedDriveLetters(Mounting, I),IncludeTrailingPathDelimiter(ExtractFilePath(CurrentGameFile)),CurrentGameName,GameDB) then exit;
          Mounting[I]:=S;
          LoadMountingList;

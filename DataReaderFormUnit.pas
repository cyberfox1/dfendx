...
begin
  ...
  if PrgSetup.HasValidExoInstallation then
    SourceComboBox.Items.Add('eXoDOS ('+Trim(PrgSetup.ExoDOSDir)+')');
end;
...
begin
  ...
  {No auto-search on open — only when user clicks Search.}
  ListBoxClick(Sender);
  ShowCompleted:=True;
...
begin
  ...
  try
    ...
    try
      ...
      ListSuccess:=ShowDataReaderInternetListWaitDialog(O,DataReader,GameNameEdit.Text,LanguageSetup.DataReaderDownloadCaption,LanguageSetup.DataReaderDownloadInfo,LanguageSetup.DataReaderDownloadError);
    finally
      Enabled:=True;
    end;
    if not ListSuccess then begin
      ListBoxClick(Sender);
      exit;
    end;
...
begin
  ...
  If (Trim(GenreSt[Nr])='') and (Trim(DeveloperSt[Nr])='') and (Trim(PublisherSt[Nr])='') and (Trim(YearSt[Nr])='') and (Trim(CoverSt[Nr])='') and (Trim(NotesSt[Nr])='') then begin
    ...
    If DataReaderGameDataThread<>nil then try
      if DataReaderGameDataThread.Meta<>nil then begin
        GenreSt[Nr]:=GetCustomGenreName(ProcessGenre(DataReaderGameDataThread.Meta.Genre));
        DeveloperSt[Nr]:=DataReaderGameDataThread.Meta.Developer;
        PublisherSt[Nr]:=DataReaderGameDataThread.Meta.Publisher;
        YearSt[Nr]:=DataReaderGameDataThread.Meta.Year;
        CoverSt[Nr]:=DataReaderGameDataThread.Meta.ImagePageURL;
  ...
  if DataReader is TExoDOSDataReader then
    NameLabel.Caption:=ExoDOSTitleWithoutYear(ListBox.Items[Nr])
  else

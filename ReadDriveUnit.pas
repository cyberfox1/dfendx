...
uses Classes, Controls, Dialogs, SysUtils, Math, LanguageSetupUnit;
...
begin
  ...
  result:=Int64(DiskGeometry.Cylinders)*DiskGeometry.TracksPerCylinder*DiskGeometry.SectorsPerTrack*DiskGeometry.BytesPerSector;
end;
...
begin
  ...
  If NativeUInt(AlignedPointer) mod Align<>0 then
    AlignedPointer:=PByte(AlignedPointer)+Align-(NativeUInt(AlignedPointer) mod Align);
end;
...
Function ReadFloppyData(const Drive : Char; const FileName : String; const OnProgress : TReadProgressEvent) : TReadDataResult;
Var Volume : THandle;
    SectorSize, BytesRead, I, TotalBytesToRead : Cardinal;
    OutputFile : TFileStream;
    RealPointer, AlignedPointer : Pointer;
    ContinueReading : Boolean;
    B : Byte;
begin
  if not GetDriveData(Drive,TotalBytesToRead,SectorSize) then begin result:=RD_CannotGetDriveData; exit; end;
  Volume:=OpenVolume(Drive);
  If Volume=INVALID_HANDLE_VALUE then begin result:=RD_CannotOpen; exit; end;
  try
    if not EnableExtendedAccess(Volume) then begin result:=RD_CannotSetExtendedAccess; exit; end;
    ...
    try
      ...
      try
        BytesRead:=0;
        while BytesRead<TotalBytesToRead do begin
          if (not ReadFile(Volume,AlignedPointer^,SectorSize,I,nil)) or (I=0) then begin
            if BytesRead=0 then begin result:=RD_ReadError; exit; end;
            break;
          end;
          OutputFile.WriteBuffer(AlignedPointer^,I);
          BytesRead:=BytesRead+I;
          If I<SectorSize then break;
          If Assigned(OnProgress) then begin
            ContinueReading:=True;
            OnProgress(BytesRead,TotalBytesToRead,ContinueReading);
            if not ContinueReading then begin result:=RD_Canceled; exit; end;
    ...
    try
      ...
      try
        ...
        while BytesToWrite>0 do begin
          InputFile.ReadBuffer(AlignedPointer^,Min(BytesToWrite,SectorSize));
          WriteFile(Volume,AlignedPointer^,Min(BytesToWrite,SectorSize),I,nil);
          If I<Min(BytesToWrite,SectorSize) then begin result:=RD_WriteError; exit; end;

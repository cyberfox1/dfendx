...
uses Windows, SysUtils, Dialogs, CommonHelpers, CommonTools, PrgSetupUnit, PrgConsts,
     LanguageSetupUnit, System.UITypes;

{ THistory }

{ The history file stores dates/times via the locale-dependent DateToStr/TimeToStr.
  When the file was written under a different locale than the one currently in use,
  the stored separator (e.g. '/') no longer matches FormatSettings.DateSeparator and
  StrToDate/StrToTime would raise EConvertError. These helpers parse without raising
  and, on failure, retry after normalizing the separators to the current locale. }

Function HistoryStrToDate(const S : String) : TDateTime;
Var T : String;
    I : Integer;
begin
  result:=0;
  T:=Trim(S);
  If T='' then exit;
  If TryStrToDate(T,result) then exit;
  For I:=1 to length(T) do If CharInSet(T[I],['/','.','-']) then T[I]:=FormatSettings.DateSeparator;
  If not TryStrToDate(T,result) then result:=0;
end;

Function HistoryStrToTime(const S : String) : TDateTime;
Var T : String;
    I : Integer;
begin
  result:=0;
  T:=Trim(S);
  If T='' then exit;
  If TryStrToTime(T,result) then exit;
  For I:=1 to length(T) do If CharInSet(T[I],['.',':']) then T[I]:=FormatSettings.TimeSeparator;
  If not TryStrToTime(T,result) then result:=0;
end;
...
begin
  ...
  try
    If I=0 then result:=DateTimeToFileDate(Rec.TimeStamp);
  finally
    FindClose(Rec);
  end;
end;
  ...
  try
    ...
    For I:=0 to RawList.Count-1 do begin
      ...
      try
        ...
        StartTime:=0;
        StartTime:=HistoryStrToDate(St[1]);
        StartTime:=StartTime+HistoryStrToTime(St[2]);
...
begin
  result:=0;
  if (I<0) or (I>StartNames.Count) then exit;
  result:=HistoryStrToDate(StartDates[I]);
  result:=result+HistoryStrToTime(StartTimes[I]);
end;
